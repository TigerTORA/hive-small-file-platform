...
class TestRulesValidator {
  ...
  async testDataRelationships(rule) {
    try {
      const response = await this.utils.page.evaluate(async () => {
        try {
          const resp = await fetch(
            "http://localhost:8000/api/v1/tables/metrics?cluster_id=1",
          );
      ...
      this.addTestResult(
        ...
        "数据关联关系",
        response.ok,
        response.ok ? "关联查询成功" : `关联查询失败: ${response.status}`,
      );
    } catch (error) {
      this.addTestResult(
        rule,
      ...
      if (clusterCards.length > 0) {
        ...
        if (detailButton) {
          ...
          const navigationSuccess =
            afterUrl !== beforeUrl && afterUrl.includes("/clusters/");
          this.addTestResult(
            rule,
            "集群详情导航",
            navigationSuccess,
            navigationSuccess ? "导航成功" : "URL未改变",
          );
        } else {
  ...
  async testBackNavigation(rule) {
    try {
      ...
      if (hasBackButton) {
        ...
        const isBackToList =
          currentUrl.includes("clusters") && !currentUrl.includes("/clusters/");

        this.addTestResult(
          rule,
          "返回功能",
          isBackToList,
          isBackToList ? "返回成功" : "返回失败",
        );
      } else {
  ...
  async testUIElementsExistence(rule) {
    const pages = [
      {
        name: "clusters",
        elements: [".cluster-card", 'button:has-text("添加集群")'],
      },
    ...
    const coverage =
      totalElements > 0 ? (foundElements / totalElements) * 100 : 0;
    this.addTestResult(
      rule,
      "UI元素覆盖率",
      coverage >= 100,
      `${coverage.toFixed(1)}%元素可访问`,
    );
  }
  ...
  async testButtonInteractions(rule) {
    try {
      ...
      const addButton = await this.utils.elementExists(
  ...
  async testDialogFunctionality(rule) {
    try {
      ...
      if (dialogExists) {
        const cancelButton = await this.utils.elementExists(
          '.el-dialog .el-button:has-text("取消")',
        );
  ...
  async testRequiredFieldValidation(rule) {
    try {
      ...
      const saveButton = await this.utils.elementExists(
  ...
  async testNewUserFlow(rule) {
    try {
      ...
      const hasWelcomeContent =
        (await this.utils.elementExists(".summary-card")) ||
      ...
      this.addTestResult(
        ...
        "新用户流程",
        hasWelcomeContent,
        hasWelcomeContent ? "首页内容正常" : "首页内容缺失",
      );
    } catch (error) {
      this.addTestResult(
        rule,
  ...
  async testCoreBusinessFlow(rule) {
    ...
    for (const flow of flows) {
      try {
        ...
        const hasContent = await this.utils.elementExists(
    ...
    try {
      ...
      const response = await this.utils.page.evaluate(async () => {
        try {
          // 尝试删除一个不存在的资源
          const resp = await fetch(
            "http://localhost:8000/api/v1/clusters/99999",
            {
              method: "DELETE",
            },
          );
          ...
          if (listResp.ok) {
            ...
            if (clusters.length > 0) {
              ...
              const deleteResp = await fetch(
                `http://localhost:8000/api/v1/clusters/${clusters[0].id}`,
                {
                  method: "DELETE",
                },
              );
  ...
  async testDependencyIntegrity(rule) {
    try {
      ...
      this.addTestResult(
        ...
        response.ok,
        response.ok
          ? `集群数: ${response.clusterCount}, 任务数: ${response.taskCount}`
          : "依赖关系检查失败",
      );
    } catch (error) {
      this.addTestResult(
  ...
  recordRuleResult(rule) {
    ...
    console.log(
      `  📊 ${rule.name}: ${rule.passed}/${rule.total} (${ruleScore.toFixed(1)}%)\n`,
    );
  }
      ...
      const status =
        ruleScore >= 80
          ...
          : ruleScore >= 60
            ...
            : "❌ 不合规";
      console.log(
        `  规则${index + 1}: ${rule.name} - ${status} (${ruleScore.toFixed(1)}%)`,
      );
    });
