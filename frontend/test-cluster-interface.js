...
async function testClusterInterface() {
  ...
  try {
    ...
    // 3. 检查页面基本元素
    const hasHeader =
      (await page.locator('h2:has-text("集群管理")').count()) > 0;
    ...
    if (clusterCards > 0) {
      // 点击第一个集群卡片
      await page
        .locator('.cluster-card .card-actions button:has-text("详情")')
        .first()
        .click();
      ...
      if (
        detailUrl.includes("/clusters/") &&
        detailUrl.split("/").length >= 5
      ) {
        ...
        const hasBackButton =
          (await page.locator('button:has-text("返回集群列表")').count()) > 0;
