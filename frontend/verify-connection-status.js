...
async function verifyConnectionStatusIndicators() {
  ...
  try {
    ...
    if (cardCount === 0) {
      ...
      const connectionIndicator = firstCard.locator(
      ...
      if (await connectionIndicator.isVisible()) {
        ...
        const serviceIndicators =
          connectionIndicator.locator(".service-indicator");
        ...
        if (indicatorCount >= 3) {
          ...
          console.log(
            `❌ 连接状态指示器数量不足，期望3个，实际${indicatorCount}个`,
          );
        }
      } else {
    ...
    const pageInfo = await page.evaluate(() => {
      return {
        ...
        connectionIndicators: document.querySelectorAll(
          ".connection-status-indicator",
        ).length,
        serviceIndicators:
          document.querySelectorAll(".service-indicator").length,
