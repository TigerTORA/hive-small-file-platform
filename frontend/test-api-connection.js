...
class ApiConnectionTester {
  ...
  buildApiEndpointsList() {
    return [
      ...
      {
        path: "/api/v1/clusters/health-check",
        method: "GET",
        category: "clusters",
        critical: false,
      },
      ...
      // 表相关API
      {
        path: "/api/v1/tables/metrics?cluster_id=1",
        method: "GET",
        category: "tables",
        critical: true,
      },
      {
        path: "/api/v1/tables/small-files?cluster_id=1",
        method: "GET",
        category: "tables",
        critical: true,
      },
      ...
      {
        path: "/api/v1/dashboard/file-distribution",
        method: "GET",
        category: "dashboard",
        critical: false,
      },
      {
        path: "/api/v1/dashboard/top-tables",
        method: "GET",
        category: "dashboard",
        critical: false,
      },
    ...
    try {
      ...
      if (healthResponse.ok) {
        ...
        this.recordTestResult(
          "api-server-health",
          true,
          `API服务器正常 (${healthResponse.status})`,
        );
      } else {
        this.utils.addTestStep(
          "API服务器健康检查",
        ...
        this.recordTestResult(
          "api-server-health",
          false,
          `API服务器异常 (${healthResponse.status})`,
        );
      }
  ...
  async testSingleApiEndpoint(endpoint) {
    ...
    try {
      ...
      const response = await this.utils.testApiEndpoint(
        endpoint.path,
        endpoint.method,
      );
      ...
      if (statusOk) {
        ...
        if (endpoint.critical) {
          this.recordTestResult(
            testName,
            false,
            `关键API异常 (${response.status})`,
          );
        } else {
          this.recordTestResult(
            testName,
            true,
            `非关键API异常 (${response.status})`,
          );
        }
      }
    ...
    try {
      ...
      if (response.status >= 500) {
        this.utils.addTestStep(
          "500错误处理",
          "success",
          `返回服务器错误: ${response.status}`,
        );
      } else {
        this.utils.addTestStep(
          "500错误测试",
          "warning",
          `无法触发500错误: ${response.status}`,
        );
      }
  ...
  async testNetworkErrorHandling() {
    ...
    try {
      ...
      // 测试连接到不存在的服务器
      const badResponse = await this.utils.testApiEndpoint(
        "/api/v1/clusters/",
        "GET",
      );
  ...
  async testClustersPageApiIntegration() {
    ...
    try {
      ...
      const hasError = await this.utils.elementExists(
        ".error-message, .el-empty",
      );
  ...
  async testDashboardPageApiIntegration() {
    ...
    try {
      ...
      // 检查仪表板组件
      const hasSummary = await this.utils.elementExists(
        ".summary-card, .dashboard-summary",
      );
      const hasCharts = await this.utils.elementExists(
  ...
  async testTablesPageApiIntegration() {
    ...
    try {
      ...
      const hasFilters = await this.utils.elementExists(
  ...
  generateSummary() {
    ...
    console.log(
      `成功率: ${((this.testResults.passed / this.testResults.total) * 100).toFixed(1)}%`,
    );
    console.log("=".repeat(50));
