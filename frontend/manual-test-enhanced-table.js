...
(async () => {
  ...
  try {
    ...
    const tablesLink = page
      .locator("text=表管理")
      .or(page.locator("text=Tables"))
      .or(page.locator('[href*="table"]'));
    if ((await tablesLink.count()) > 0) {
      ...
      // 查找可点击的表名
      const clickableTableNames = page.locator(
        '.table-name-link, .clickable-table-name, [class*="table-name"][class*="link"], a[href*="table"], .el-link',
      );
      ...
      if (count > 0) {
        ...
        const allLinks = page.locator(
          'a, .el-link, [role="button"], .clickable',
        );
