  ...
  try {
    ...
    if (linkCount > 0) {
      ...
      if (currentUrl.includes('/tables/')) {
        ...
        // 检查表类型信息
        const managedTableTag = page.locator('.el-tag:has-text("托管表"), .el-tag:has-text("外部表")');
        ...
        // 检查存储格式
        const storageFormatTags = page.locator('.el-tag:has-text("ORC"), .el-tag:has-text("TEXT"), .el-tag:has-text("PARQUET")');
        ...
        // 检查分区信息
        const partitionTag = page.locator('.el-tag:has-text("是"), .el-tag:has-text("否")');
        ...
        if (await recommendationsSection.count() > 0) {
          ...
          // 检查小文件警告
          const smallFileAlert = page.locator('.el-alert:has-text("小文件问题")');
        ...
        // 点击面包屑导航返回表管理页面
        const tablesLink = page.locator('.el-breadcrumb-item:has-text("表管理") a');
        ...
        for (let i = 1; i < maxTablesToTest; i++) {
          ...
          // 返回表管理页面
          const backButton = page.locator('.el-breadcrumb-item:has-text("表管理") a');
