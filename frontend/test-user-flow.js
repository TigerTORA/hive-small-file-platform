...
;(async () => {
  ...
  try {
    ...
    if (currentUrl.includes('/clusters')) {
      ...
      // 检查是否有集群选择提示
      const noticeExists = (await page.locator('.cluster-selection-notice').count()) > 0
      if (noticeExists) {
      ...
      if (clusterCards > 0) {
        ...
        // 获取第一个集群的名称
        const clusterName = await page.locator('.cluster-card .cluster-name').first().textContent()
        ...
        if (newUrl.includes('/#/') && !newUrl.includes('clusters')) {
          ...
          // 检查页面标题
          const pageTitle = await page.locator('.dashboard-title').textContent()
          ...
          if (valueMetrics > 0) {
            ...
            for (let i = 0; i < Math.min(valueMetrics, 4); i++) {
              const card = page.locator('.value-metrics-grid .cloudera-metric-card').nth(i)
            ...
            // 查找集群切换器
            const clusterSwitcher = await page.locator('.cluster-switcher').count()
            ...
            if (selectedCluster) {
              ...
              console.log('💾 存储的集群ID:', clusterData.settings?.selectedCluster)
            } else {
