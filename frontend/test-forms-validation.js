...
class FormValidationTester {
  ...
  async openClusterDialog() {
    ...
    try {
      const success = await this.utils.clickElement(
  ...
  async testRequiredFieldValidation() {
    ...
    try {
      ...
      // 检查验证错误
      const errorExists = await this.utils.elementExists(
        ".el-form-item__error",
      );
      if (errorExists) {
  ...
  async testUrlFormatValidation() {
    ...
    try {
      ...
      // 测试有效的URL
      await this.testValidUrl(
        selectors.metastoreInput,
        TEST_CONFIG.testData.cluster.hive_metastore_url,
        "MetaStore URL",
      );
      await this.testValidUrl(
        selectors.hdfsInput,
        TEST_CONFIG.testData.cluster.hdfs_namenode_url,
        "HDFS URL",
      );
  ...
  async testFormResetFunctionality() {
    ...
    try {
      ...
      // 检查是否有重置按钮
      const resetButton = await this.utils.elementExists(
  ...
  async testTaskFormFields() {
    ...
    if (await this.utils.elementExists(taskNameInput)) {
      await this.utils.page.fill(
        taskNameInput,
        TEST_CONFIG.testData.task.task_name,
      );
    ...
    if (await this.utils.elementExists(tableNameInput)) {
      await this.utils.page.fill(
        tableNameInput,
        TEST_CONFIG.testData.task.table_name,
      );
    ...
    if (await this.utils.elementExists(strategySelect)) {
      ...
      // 选择第一个选项
      const options = await this.utils.elementExists(
  ...
  async testSettingsFormValidation() {
    ...
    // 检查设置表单是否存在
    const configForm = await this.utils.elementExists(
      TEST_CONFIG.selectors.settings.configForm,
    );
    if (configForm) {
    ...
    try {
      ...
      if (inputs.length > 0) {
        ...
        const saveButton = await this.utils.elementExists(
          TEST_CONFIG.selectors.settings.saveButton,
        );
        if (saveButton) {
          // 不实际点击，只检查按钮存在
  ...
  async closeDialog() {
    try {
      const cancelButton = await this.utils.elementExists(
        TEST_CONFIG.selectors.clusters.cancelButton,
      );
      ...
      await this.utils.page.waitForSelector(".el-dialog", {
        state: "hidden",
        timeout: 3000,
      });
    } catch (error) {
