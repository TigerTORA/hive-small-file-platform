...
class TestDataManager {
    ...
    try {
      const testClusters = [
        {
          ...
          hive_metastore_url: 'mysql://testuser:testpass@localhost:3306/test_hive_dev',
        ...
        },
        {
          ...
          hive_metastore_url: 'mysql://testuser:testpass@localhost:3306/test_hive_prod',
      ...
      for (const clusterData of testClusters) {
        ...
        if (clusterId) {
          ...
          this.utils.addTestStep('创建测试集群', 'success', `集群: ${clusterData.name}`);
        } else {
          this.utils.addTestStep('创建测试集群', 'failed', `集群创建失败: ${clusterData.name}`);
        }
      }
      
      this.utils.finishTest('success');
      this.recordTestResult('create-test-clusters', true, `创建了${this.createdClusters.length}个测试集群`);
  ...
  async createSingleCluster(clusterData) {
    try {
      ...
      console.log(`📊 创建前集群数量: ${beforeResponse.ok ? beforeResponse.count : 'unknown'}`);
      
      // 通过API直接创建集群
      const apiResponse = await this.utils.page.evaluate(async (clusterData) => {
        try {
          const createPayload = {
      ...
      if (apiResponse.ok && apiResponse.data) {
        ...
        console.error(`❌ API创建集群失败: ${apiResponse.error || apiResponse.status}`);
  ...
  async createTestTasks() {
    ...
    try {
      ...
      if (this.createdClusters.length === 0) {
        ...
        this.utils.finishTest('success');
        this.recordTestResult('create-test-tasks', true, '跳过任务创建（无集群）');
        return;
      }
  ...
  async createSampleTasks() {
    ...
    for (const taskData of testTasks) {
      ...
      if (taskId) {
        ...
        this.utils.addTestStep('创建测试任务', 'success', `任务: ${taskData.task_name}`);
      }
    }
  }
  ...
  async verifyClusterData() {
    ...
    if (response.ok) {
      ...
      this.utils.addTestStep('集群数据验证', 'warning', `API状态: ${response.status}`);
    }
  ...
  async verifyTaskData() {
    ...
    if (response.ok) {
      ...
      this.utils.addTestStep('任务数据验证', 'warning', `API状态: ${response.status}`);
    }
  ...
  async verifyTableData() {
    ...
    if (response.ok) {
      ...
      this.utils.addTestStep('表数据验证', 'warning', `API状态: ${response.status}`);
    }
  }
          ...
          if (titleElement) {
            ...
            if (title && (title.includes('测试') || title.includes('AutoTest'))) {
              ...
              if (deleteButton) {
                ...
                const confirmButton = await this.utils.elementExists('.el-message-box .el-button--primary');
                if (confirmButton) {
                  await this.utils.clickElement('.el-message-box .el-button--primary');
                  await this.utils.page.waitForTimeout(1000);
                }
                
                this.utils.addTestStep('删除测试集群', 'success', `已删除: ${title}`);
              }
            }
          }
        } catch (error) {
          ...
          if (nameElement) {
            ...
            if (name && name.includes('测试任务')) {
              ...
              if (deleteButton) {
                await deleteButton.click();
                await this.utils.page.waitForTimeout(1000);
                this.utils.addTestStep('删除测试任务', 'success', `已删除: ${name}`);
              }
            }
          }
        } catch (error) {
  ...
  async verifyCleanupCompletion() {
    ...
    try {
      ...
      this.utils.addTestStep('集群清理验证', 'success', `剩余测试集群: ${remainingClusters}`);
      ...
      this.utils.addTestStep('任务清理验证', 'success', `剩余测试任务: ${remainingTasks}`);
