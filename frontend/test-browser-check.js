  ...
  for (const page of pages) {
    try {
      ...
      testResults.frontend[page.name] = {
        status: result.statusCode === 200 && result.data.includes('id="app"') ? 'OK' : 'FAILED',
...
async function testPerformance() {
  ...
  for (const test of performanceTests) {
    ...
    console.log(`  📊 ${test.name}: 平均 ${Math.round(avgTime)}ms (${minTime}-${maxTime}ms)`)
  }
}
...
function generateReport() {
  ...
  Object.entries(testResults.frontend).forEach(([name, result]) => {
    ...
    console.log(`  ${status} ${name}: ${result.status} (${result.responseTime || 'N/A'}ms)`)
  })
  ...
  Object.entries(testResults.performance).forEach(([name, result]) => {
    const emoji = result.status === 'GOOD' ? '🟢' : result.status === 'ACCEPTABLE' ? '🟡' : '🔴'
    console.log(`  ${emoji} ${name}: ${result.averageTime}ms (${result.status})`)
  })
  ...
  // 总结
  const frontendOK = Object.values(testResults.frontend).filter(r => r.status === 'OK').length
  const apiOK = Object.values(testResults.api).filter(r => r.status === 'OK').length
  const routesOK = Object.values(testResults.routes).filter(r => r.status === 'OK').length
  const perfGood = Object.values(testResults.performance).filter(r => r.status === 'GOOD').length

  console.log('\n📊 测试总结:')
  console.log(`  • 前端页面: ${frontendOK}/${Object.keys(testResults.frontend).length} 通过`)
  console.log(`  • API接口: ${apiOK}/${Object.keys(testResults.api).length} 通过`)
  console.log(`  • URL路由: ${routesOK}/${Object.keys(testResults.routes).length} 通过`)
  console.log(`  • 性能良好: ${perfGood}/${Object.keys(testResults.performance).length} 项`)
