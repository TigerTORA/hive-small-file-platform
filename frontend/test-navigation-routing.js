...
class NavigationRoutingTester {
  ...
  async testSinglePageNavigation(pageName) {
    ...
    try {
      ...
      const urlContainsPath = currentUrl.includes(
      ...
      // 验证页面标题
      const titleCorrect = await this.utils.verifyPageTitle(
        this.getExpectedTitle(pageName),
      );
      if (titleCorrect) {
  ...
  async testClusterDetailRouting() {
    ...
    try {
      ...
      // 点击进入集群详情
      const detailButton =
        ".cluster-actions button:first-child, .cluster-card .el-button--primary";
  ...
  async testTableDetailRouting() {
    ...
    try {
      ...
      // 检查是否有表数据
      const tableExists = await this.utils.elementExists(
  ...
  async testInvalidRoute() {
    ...
    const invalidUrl =
      TEST_CONFIG.app.baseUrl + "/#/invalid-page-that-does-not-exist";
