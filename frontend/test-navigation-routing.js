...
class NavigationRoutingTester {
  ...
  async testSinglePageNavigation(pageName) {
    ...
    try {
      ...
      if (urlContainsPath) {
        ...
        this.utils.addTestStep('验证URL', 'failed', `URL不匹配: 期待${expectedPath}, 实际${currentUrl}`);
      }
  ...
  async testClusterDetailRouting() {
    ...
    try {
      ...
      if (await this.utils.elementExists(detailButton)) {
        ...
        const currentUrl = this.utils.page.url();
        const hasClusterId = currentUrl.includes('#/clusters/') && /#\/clusters\/\d+/.test(currentUrl);
    ...
    try {
      ...
      for (const page of pages) {
        ...
        if (title.includes(expectedTitle)) {
          ...
          this.utils.addTestStep(`${page}页面标题`, 'warning', `期待包含'${expectedTitle}'，实际'${title}'`);
        }
      }
  ...
  async testInvalidRoute() {
    ...
    try {
      ...
      const isRedirected = currentUrl.includes('clusters') || 
