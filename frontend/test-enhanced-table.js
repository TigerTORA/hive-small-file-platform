...
async function testEnhancedTable() {
  ...
  try {
    ...
    if (sizeCount > 0) {
      ...
      if (
        firstSizeValue &&
        (firstSizeValue.includes("B") ||
          firstSizeValue.includes("K") ||
          firstSizeValue.includes("M") ||
          firstSizeValue.includes("G"))
      ) {
    ...
    console.log("\n📋 测试5: 验证平均文件大小列");
    // 查找平均文件大小列的数据
    const avgSizeCells = page.locator(
      'td[data-column="avg_file_size"], td:nth-child(6)',
    );
    const avgSizeCount = await avgSizeCells.count();
    if (avgSizeCount > 0) {
      const firstAvgSize = await avgSizeCells.first().textContent();
      console.log(
        `✅ 找到 ${avgSizeCount} 个平均文件大小值，示例: ${firstAvgSize}`,
      );
    }
    ...
    if (foundColorClasses > 0) {
      console.log(
        `✅ 小文件占比颜色编码工作正常，发现 ${foundColorClasses} 种不同颜色级别`,
      );
    }
