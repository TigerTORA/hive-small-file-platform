  ...
  try {
    ...
    if (clusterCards === 0) {
      ...
      await page.evaluate(() => {
        ...
        const grid =
          document.querySelector(".clusters-grid") ||
          document.querySelector(".clusters-management");
        if (grid) {
    ...
    const cardHeights = await page.evaluate(() => {
      const cards = document.querySelectorAll(
        ".cluster-card, .test-cluster-card",
      );
    ...
    cardHeights.forEach((height, index) => {
      ...
      if (height.actualHeight <= 260 && height.actualHeight >= 220) {
        ...
        console.log(
          `   ⚠️ 高度可能需要调整 - 当前 ${height.actualHeight.toFixed(1)}px`,
        );
      }
    });
    ...
    const operationsLayout = await page.evaluate(() => {
      ...
      return Array.from(operations).map((op) => {
        ...
        return {
          ...
          buttonTexts: Array.from(buttons).map(
    ...
    operationsLayout.forEach((layout, index) => {
      ...
      if (hasConnectionTest) {
        const isOptimizedText = layout.buttonTexts.some((text) =>
          text.includes("连接测试"),
        );
        if (isOptimizedText) {
    ...
    const report = {
      ...
      verificationResults: {
        cardHeight: {
          ...
          status: cardHeights.every(
            (h) => h.actualHeight <= 260 && h.actualHeight >= 220,
          )
            ? "PASS"
            : "REVIEW",
        },
        nameAndStatusLayout: {
          ...
          status: nameRowLayout.every(
            (l) => l.display === "flex" && l.childrenCount >= 2,
          )
            ? "PASS"
            : "REVIEW",
        },
        statsLayout: {
          target: "统计信息水平紧凑布局",
          status: statsLayout.every(
            (l) => l.display === "flex" && l.statItemsCount >= 3,
          )
            ? "PASS"
            : "REVIEW",
        },
        operationsOptimization: {
          ...
          status: operationsLayout.some((l) => l.dropdownsCount > 0)
            ? "PASS"
            : "REVIEW",
        },
      },
    ...
    const reportPath = path.join(
      __dirname,
      "docs/cluster-card-optimization-report.json",
    );
    ...
    console.log(
      `📊 验证成功率: ${((passCount / totalCount) * 100).toFixed(1)}%`,
    );
    ...
    const errorScreenshotPath = path.join(
      __dirname,
      "docs/screenshots/cluster-cards-error.png",
    );
    await page.screenshot({
