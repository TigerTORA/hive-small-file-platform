...
export const tasksApi = {
  ...
  getTablePartitions(
    ...
    return api.get(
      `/tables/${clusterId}/${databaseName}/${tableName}/partitions`,
    );
  },
  ...
  getMergePreview(
    ...
    const params = partitionFilter
      ? `?partition_filter=${encodeURIComponent(partitionFilter)}`
      : "";
    return api.post(
      `/tables/${clusterId}/${databaseName}/${tableName}/merge-preview${params}`,
    );
  },
  ...
  scanAllDatabases(
    clusterId: number,
    maxTablesPerDb: number = 20,
  ): Promise<any> {
    return api.post(
      `/tables/scan-all-databases/${clusterId}?max_tables_per_db=${maxTablesPerDb}`,
    );
  },
