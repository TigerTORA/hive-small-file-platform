      ...
      >
        ...
        >
          <template #default="{ row }">
            ...
            <el-tag
              v-else-if="row.days_since_last_access > 0"
              type="success"
              size="small"
            >
...
<script setup lang="ts">
  ...
  const archiveTable = async (table: TableMetric) => {
    try {
      ...
      const response = await tablesApi.archiveTable(
        selectedCluster.value,
        table.database_name,
        table.table_name,
        true
  ...
  const restoreTable = async (table: TableMetric) => {
    try {
      ...
      const response = await tablesApi.restoreTable(
        selectedCluster.value,
        table.database_name,
        table.table_name
      )
  ...
  const triggerColdDataScan = async () => {
    ...
    try {
      ...
      ElMessage.success(
        `冷数据扫描完成！发现 ${response.scan_result?.cold_tables_found || 0} 个冷数据表`
      )
