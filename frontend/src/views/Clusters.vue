        ...
        <div class="card-header">
          ...
          <div class="header-actions">
            <el-dropdown @command="handleTemplate">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item command="cdp"
                    >CDP 集群模板</el-dropdown-item
                  >
                  <el-dropdown-item command="cdh"
                    >CDH 集群模板</el-dropdown-item
                  >
                  <el-dropdown-item command="hdp"
                    >HDP 集群模板</el-dropdown-item
                  >
                </el-dropdown-menu>
              </template>
            </el-dropdown>
    ...
    try {
      ...
      if (result.connections?.metastore?.status === "success") {
        ...
        if (hdfsStatus === "success") {
          ...
          ElMessage.warning(
            `MetaStore连接成功，但HDFS连接失败 (${hdfsMode}模式)`,
          );
        }
      } else {
        ElMessage.error(
          `连接测试失败: ${result.connections?.metastore?.message || "未知错误"}`,
        );
      }
    } catch (error: any) {
      console.error("Failed to test connection:", error);
...
const clusterTemplates = {
  ...
  cdh: {
    ...
    hive_database: "default",
    hive_metastore_url:
      "postgresql://hive:password@cdh-master:5432/hive_metastore",
...
const applyTemplate = (templateName: string) => {
  if (
    templateName &&
    clusterTemplates[templateName as keyof typeof clusterTemplates]
  ) {
    const template =
      clusterTemplates[templateName as keyof typeof clusterTemplates];
    clusterForm.value = { ...template };
    ElMessage.info(
      `已应用 ${templateName.toUpperCase()} 集群模板，请根据实际情况修改配置`,
    );
  }
};
