<template>
  <div class="test-dashboard">
        ...
        >
          ...
          >
            <template #default="scope">
              ...
              <el-button
                size="small"
                @click="viewTestDetails(scope.row)"
              >
    ...
    >
      <div
        v-if="selectedScreenshots.length > 0"
...
<script setup>
  ...
  const filteredTestResults = computed(() => {
    ...
    if (filterCategory.value) {
      filtered = filtered.filter(test => test.category === getCategoryName(filterCategory.value))
    }
  ...
  const refreshData = async () => {
    ...
    try {
      // 刷新所有数据
      await Promise.all([loadTestOverview(), loadTestCategories(), loadTestResults()])
