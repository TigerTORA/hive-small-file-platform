<template>
  <div class="cluster-detail">
    ...
    <el-card class="cluster-header">
      <div class="header-content">
        <div class="cluster-info">
          ...
          <div class="cluster-meta" v-if="cluster">
    ...
    <div class="stats-grid">
      ...
      <el-card>
        <el-statistic title="总表数" :value="stats.total_tables" suffix="张">
          <template #prefix>
    ...
    <el-card class="main-content">
      <el-tabs v-model="activeTab" type="border-card">
        ...
        <el-tab-pane label="小文件分析" name="analysis">
          <SmallFilesAnalysis :cluster-id="clusterId" />
        </el-tab-pane>

        <el-tab-pane label="设置" name="settings">
          <ClusterSettings :cluster-id="clusterId" @updated="loadClusterInfo" />
        </el-tab-pane>
      </el-tabs>
    </el-card>

    <!-- Scan Database Dialog -->
    <el-dialog v-model="showScanDialog" title="扫描数据库" width="500px">
      <el-form :model="scanForm" label-width="100px">
        ...
        <el-form-item label="数据库名" v-if="scanForm.mode === 'single'">
          <el-select
...
const loadStats = async () => {
  try {
    ...
    const errorMsg =
      error.response?.data?.detail || error.message || "加载统计数据失败";
...
const loadDatabases = async () => {
  try {
    ...
    const errorMsg =
      error.response?.data?.detail || error.message || "加载数据库列表失败";
...
const testConnection = async () => {
  ...
  try {
    ...
    testError.value =
      error.response?.data?.detail || error.message || "测试失败";
  } finally {
...
const startScan = async () => {
  ...
  try {
    ...
    if (scanForm.value.mode === "all") {
      ...
      response = await tablesApi.scanDatabase(
        clusterId.value,
        scanForm.value.database,
      );
    }
    ...
    const errorMsg =
      error.response?.data?.detail || error.message || "扫描启动失败";
