      ...
      >
        ...
        <div class="cluster-content">
          ...
          <div class="cluster-operations" @click.stop>
            <el-button
              size="small"
              @click="testConnection(cluster, 'mock')"
...
const totalTables = computed(() =>
  Object.values(clusterStats.value).reduce(
    (sum, stat) => sum + (stat.tables || 0),
    0,
  ),
);
const pendingTasks = computed(() =>
  Object.values(clusterStats.value).reduce(
    (sum, stat) => sum + (stat.pending_tasks || 0),
    0,
  ),
);
...
const testConnection = async (cluster: Cluster, mode: string = "mock") => {
  if (mode === "real") {
    ...
    try {
      ...
      console.error("Failed to test connection:", error);
      const errorMsg =
        error.response?.data?.detail || error.message || "网络连接失败";
