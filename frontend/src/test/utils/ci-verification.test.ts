...
describe('CI流水线验证测试', () => {
  ...
  describe('企业级标准合规性测试', () => {
    ...
    it('应该验证测试文件结构', () => {
      ...
      expect(testFileStats.totalTests).toBeGreaterThanOrEqual(testFileStats.minRequired)
  ...
  describe('模拟集成测试场景', () => {
    it('应该模拟集群状态检查', () => {
      ...
      expect(clusterStatus.smallFiles).toBeLessThanOrEqual(clusterStatus.totalFiles)
      expect(clusterStatus.coverageAchieved).toBeGreaterThanOrEqual(0.75)

      // 验证小文件比例
      const smallFileRatio = clusterStatus.smallFiles / clusterStatus.totalFiles
