...
test.describe("Clusters Management Page", () => {
  ...
  test("应该显示集群管理页面", async ({ page }) => {
    ...
    const clustersContainer = page.locator(
      ".clusters-container, .el-table, .cluster-list",
    );
    ...
    if (clusterCount > 0) {
      ...
      if (await testButton.isVisible()) {
        ...
        const resultMessage = page.locator(
          ".el-message, .el-notification, .result-message",
        );
  ...
  test("应该显示集群状态信息", async ({ page }) => {
    ...
    const statusElements = page.locator(
      '.el-tag, .status-indicator, [data-testid="cluster-status"]',
    );
    ...
    if ((await statusElements.count()) > 0) {
      ...
      await expect(firstStatus).toContainText(
        /(活跃|离线|连接中|已连接|未连接|active|inactive|online|offline)/,
      );
    }
  });
  ...
  test("应该支持集群搜索和过滤", async ({ page }) => {
    ...
    const searchInput = page.locator(
      'input[placeholder*="搜索"], input[placeholder*="集群名称"], .el-input__inner',
    );
    ...
    if (await searchInput.first().isVisible()) {
      ...
      expect(
        await page.locator(".el-table, .cluster-list").isVisible(),
      ).toBeTruthy();
    }
  });
  ...
  test("应该处理集群管理操作", async ({ page }) => {
    ...
    if ((await actionButtons.count()) > 0) {
      ...
      if (await editButton.isVisible()) {
        ...
        expect(
          (await dialog.isVisible()) || (await page.url().includes("edit")),
        ).toBeTruthy();
      }
    }
  });
  ...
  test("应该显示集群连接配置", async ({ page }) => {
    ...
    if (await addButton.isVisible()) {
      ...
      const formFields = page.locator(
        'input[placeholder*="集群名称"], input[placeholder*="主机"], input[placeholder*="端口"]',
      );
    ...
    if (await addButton.isVisible()) {
      ...
      if (await submitButton.isVisible()) {
        ...
        const errorMessages = page.locator(
          ".el-form-item__error, .error-message, .validation-error",
        );
