...
test.describe("关键用户场景测试", () => {
  ...
  test.describe("场景1：系统管理员日常监控", () => {
    test("应该完成完整的监控工作流程", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="big-screen-time"]'),
      ).toBeVisible();
    ...
    test("应该正确响应集群切换", async ({ page }) => {
      ...
      // 切换集群（如果有多个集群选项）
      const clusterSelect = page.locator(
        ".cluster-selector select, .el-select",
      );
  ...
  test.describe("场景2：运维人员任务执行", () => {
    test("应该完成扫描任务创建和监控流程", async ({ page }) => {
      ...
      if (await createTaskBtn.isVisible()) {
        ...
        // 确认创建
        await page
          .locator('button:has-text("确定"), button:has-text("创建")')
          .click();

        // 验证任务创建成功
        await expect(
          page.locator(".el-message--success, .success-message"),
        ).toBeVisible();
      }
      ...
      if ((await taskItems.count()) > 0) {
        ...
        await expect(taskItems.first()).toContainText(
    ...
    test("应该正确显示任务执行日志", async ({ page }) => {
      ...
      const logButton = page
        .locator('text=日志, button:has-text("查看日志")')
        .first();
      if (await logButton.isVisible()) {
        ...
        if (await logContent.isVisible()) {
          await expect(logContent).toContainText(
            /INFO|ERROR|WARN|开始|完成|处理/,
          );
        }

        // 关闭日志对话框
        await page
          .locator('.el-dialog__close, button:has-text("关闭")')
          .click();
      }
    });
  });
  ...
  test.describe("场景3：数据分析师表管理", () => {
    ...
    test("应该正确显示表的文件分布信息", async ({ page }) => {
      // 直接访问一个表详情页面（演示数据）
      await page.goto(
        `${APP_URL}/#/tables/cluster-1/demo_db/demo_table?demo=true`,
      );
      await page.waitForLoadState("networkidle");

      // 验证基本信息
      await expect(
        page.locator(".basic-info, .table-basic-info"),
      ).toBeVisible();
      ...
      const chartContainer = page.locator(
        ".chart-container, .file-distribution-chart",
      );
  ...
  test.describe("场景4：特性开关和配置管理", () => {
    ...
    test("应该支持演示模式切换", async ({ page }) => {
      ...
      const demoIndicator = page.locator(
        '.demo-mode-indicator, [data-testid="demo-mode"]',
      );
    ...
    test("应该提供清晰的反馈信息", async ({ page }) => {
      ...
      if (await actionButton.isVisible()) {
        ...
        const feedback = page.locator(
          ".el-message, .loading, .success-message",
        );
