...
test.describe('关键用户场景测试', () => {
  ...
  test.describe('场景2：运维人员任务执行', () => {
    test('应该完成扫描任务创建和监控流程', async ({ page }) => {
      ...
      // 验证任务列表加载（适配新版统一表格）
      await expect(page.locator('[data-testid="tasks-table"], .cloudera-data-table')).toBeVisible()
  ...
  test.describe('场景3：数据分析师表管理', () => {
    test('应该完成表查询和详情查看流程', async ({ page }) => {
      ...
      // 验证表列表加载
      await expect(page.locator('[data-testid="tables-page"], .cloudera-data-table')).toBeVisible()
