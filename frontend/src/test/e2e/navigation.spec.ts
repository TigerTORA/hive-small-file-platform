...
test.describe('Application Navigation', () => {
  ...
  test('应该支持页面间导航', async ({ page }) => {
    ...
    for (const nav of navigationTests) {
      ...
      const navLink = page.locator(`a[href="${nav.url}"], .el-menu-item`).filter({ hasText: nav.text })
  ...
  test('应该显示页面标题和面包屑', async ({ page }) => {
    ...
    if (await breadcrumb.isVisible()) {
      const breadcrumbItems = breadcrumb.locator('.el-breadcrumb-item, .breadcrumb-item')
  ...
  test('应该支持侧边栏折叠展开', async ({ page }) => {
    ...
    const toggleButton = page.locator('.sidebar-toggle, .menu-toggle, [aria-label*="toggle"], [aria-label*="折叠"]')
  ...
  test('应该显示用户信息和操作', async ({ page }) => {
    ...
    if (await userInfo.isVisible()) {
      ...
      const userDropdown = page.locator('.el-dropdown-menu, .user-dropdown, .dropdown-menu')
      
      if (await userDropdown.isVisible()) {
        // Check for common user actions
        const userActions = userDropdown.locator('text=设置, text=退出, text=个人中心')
...
test.describe('Error Handling and Edge Cases', () => {
  test('应该处理404页面', async ({ page }) => {
    ...
    const is404 = await page.locator('text=404, text=页面不存在, text=Not Found').isVisible()
  ...
  test('应该处理网络连接问题', async ({ page }) => {
    ...
    const offlineIndicator = page.locator('text=网络连接失败, text=离线, text=连接错误, .offline-indicator')
    const pageStillFunctional = page.locator('body').isVisible()
    
    // Either shows offline message or page remains functional
    expect(await offlineIndicator.isVisible() || await pageStillFunctional).toBeTruthy()
  ...
  test('应该处理长时间加载', async ({ page }) => {
    ...
    const loadingIndicator = page.locator('.el-loading, .loading, [data-loading="true"], .el-skeleton')
...
test.describe('Accessibility and Usability', () => {
  ...
  test('应该有适当的语义化标签', async ({ page }) => {
    ...
    const semanticElements = page.locator('main, nav, header, section, article')
