...
test.describe("Dashboard Page", () => {
  ...
  test("应该显示加载状态", async ({ page }) => {
    ...
    const loadingIndicators = page.locator(
      '.el-skeleton, .loading, [data-loading="true"]',
    );
  ...
  test("应该处理网络错误情况", async ({ page }) => {
    ...
    const errorElements = page.locator(
      '.el-empty, .error-message, [data-testid="error"]',
    );
    const hasError = (await errorElements.count()) > 0;

    // Should either show error state or gracefully handle the failure
    expect(
      hasError || (await page.locator(".main-content").isVisible()),
    ).toBeTruthy();
  });
  ...
  test("应该支持响应式设计", async ({ page }) => {
    ...
    expect(
      (await mobileMenu.isVisible()) || (await navigation.isVisible()),
    ).toBeTruthy();
  });
