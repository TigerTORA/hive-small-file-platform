...
describe("性能基准测试", () => {
  ...
  describe("异步操作性能", () => {
    ...
    it("并发API加载性能", async () => {
      ...
      (dashboardApi.getClusterStats as any).mockImplementation(
        () =>
          new Promise((resolve) =>
            setTimeout(() => resolve({ clusters: [] }), mockDelay),
          ),
      );
  ...
  describe("性能监控和度量", () => {
    ...
    it("应该检测性能回归", () => {
      ...
      expect(currentPerformance.storeInit).toBeLessThan(
        baseline.storeInit * 1.2,
      );
      expect(currentPerformance.dataProcessing).toBeLessThan(
        baseline.dataProcessing * 1.2,
      );
      expect(currentPerformance.apiCall).toBeLessThan(baseline.apiCall * 1.2);
      expect(currentPerformance.formatting).toBeLessThan(
        baseline.formatting * 1.2,
      );
