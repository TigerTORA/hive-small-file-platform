...
describe("Dashboard Store Integration Tests", () => {
  ...
  describe("数据加载功能", () => {
    ...
    it("应该能够加载最近任务数据", async () => {
      const mockRecentTasks = [
        {
          task_id: 1,
          task_name: "merge_task_1",
          status: "completed",
          created_at: "2023-12-01",
        },
        {
          task_id: 2,
          task_name: "merge_task_2",
          status: "running",
  ...
  describe("错误处理", () => {
    it("应该正确处理加载概要数据的错误", async () => {
      ...
      const { dashboardApi } = await import("@/api/dashboard");
      (dashboardApi.getSummary as any).mockRejectedValue(
  ...
  describe("集成场景测试", () => {
    it("应该支持完整的数据加载工作流", async () => {
      ...
      (dashboardApi.getRecentTasks as any).mockResolvedValue(
        mockData.recentTasks,
      );
      (dashboardApi.getClusterStats as any).mockResolvedValue(
        mockData.clusterStats,
      );
    ...
    it("应该能够处理混合成功失败的加载场景", async () => {
      ...
      (dashboardApi.getTrends as any).mockRejectedValue(
        new Error("Trends API failed"),
      );
      (dashboardApi.getFileDistribution as any).mockResolvedValue([]);
      (dashboardApi.getTopTables as any).mockRejectedValue(
        new Error("TopTables API failed"),
      );
      (dashboardApi.getRecentTasks as any).mockResolvedValue([]);
      (dashboardApi.getClusterStats as any).mockResolvedValue({ clusters: [] });
