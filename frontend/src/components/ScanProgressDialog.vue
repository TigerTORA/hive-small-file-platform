      ...
      <div class="progress-bar-section">
        ...
        >
          <span class="progress-text"
            >{{ progress?.progress_percentage || 0 }}%</span
          >
        </el-progress>
        ...
        <div class="cloudera-table logs-container-wrapper">
          <div class="logs-container" ref="logsContainer">
            ...
            >
              ...
              <span class="cloudera-tag" :class="getLogType(log.level)">{{
                log.level
              }}</span>
              <span class="log-message">{{ log.message }}</span>
              <span v-if="log.database_name" class="log-db"
...
const scanStats = computed(() => ({
  tables:
    progress.value?.logs?.filter(
      (log) => log.level === "INFO" && log.message.includes("扫描完成"),
    ).length || 0,
...
const hasStats = computed(
  () =>
    progress.value &&
    (progress.value.completed_items > 0 || scanStats.value.tables > 0),
);
...
const cancelTask = async () => {
  if (!props.taskId) return;
  try {
    await ElMessageBox.confirm("确定要取消当前扫描任务吗？", "确认取消", {
      type: "warning",
    });
  } catch {
    return;
