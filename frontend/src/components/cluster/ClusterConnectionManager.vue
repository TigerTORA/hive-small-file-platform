...
const testConnectionEnhanced = async (
  ...
  try {
    const result = await clustersApi.testConnectionEnhanced(
      cluster.id,
      options,
    );
    ...
    testError.value =
      error.response?.data?.detail || error.message || "增强连接测试失败";
  } finally {
...
const updateConnectionStatus = async (clusterId: number) => {
  ...
  try {
    ...
    console.error(
      `Failed to check connection status for cluster ${clusterId}:`,
      error,
    );
    connectionStatus.value[clusterId] = {
...
const checkAllConnectionStatus = async () => {
  const promises = props.clusters.map((cluster) =>
    updateConnectionStatus(cluster.id),
  );
