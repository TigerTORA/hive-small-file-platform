    ...
    >
      <div class="controls-left">
        ...
        <el-text
          ...
          class="controls-desc"
        >
          {{ isEditMode ? '拖拽卡片重新排列，调整大小' : '点击编辑按钮开始自定义布局' }}
        </el-text>
      </div>
...
<script setup lang="ts">
  ...
  const canReset = computed(() => {
    return JSON.stringify(currentLayout.value) !== JSON.stringify(originalLayout.value)
  })
...
<style scoped>
  ...
  :deep(.vue-grid-placeholder) {
    background: linear-gradient(45deg, rgba(64, 158, 255, 0.05), rgba(64, 158, 255, 0.15));
