...
class CRUDOperationsTester {
  ...
  async createTestCluster() {
    try {
      ...
      const clusterData = {
        ...
        hive_metastore_url:
          "mysql://testuser:testpass@localhost:3306/test_hive",
      ...
      if (response.ok && response.data) {
        ...
        this.recordOperation(
          "创建集群",
          true,
          `成功创建集群 ID: ${this.testClusterId}`,
        );
        console.log(
          `  ✅ 集群创建成功: ${clusterData.name} (ID: ${this.testClusterId})`,
        );
      } else {
      ...
      for (const taskData of taskTemplates) {
        ...
        if (response.ok && response.data) {
          ...
          this.recordOperation(
            "创建合并任务",
            true,
            `成功创建任务 ID: ${response.data.id}`,
          );
          console.log(
            `  ✅ 合并任务创建成功: ${taskData.task_name} (ID: ${response.data.id})`,
          );
        } else {
          this.recordOperation(
            "创建合并任务",
  ...
  async verifyCreateResults() {
    try {
      // 验证集群是否存在
      const clusterExists = await this.utils.page.evaluate(
        async (clusterId) => {
          try {
            const resp = await fetch(
              `http://localhost:8000/api/v1/clusters/${clusterId}`,
            );
            return resp.ok;
          } catch (error) {
            return false;
  ...
  async testReadAssociatedTasks() {
    ...
    try {
      const response = await this.utils.page.evaluate(async (clusterId) => {
        try {
          const resp = await fetch(
            `http://localhost:8000/api/v1/tasks/?cluster_id=${clusterId}`,
          );
          if (resp.ok) {
            const data = await resp.json();
            return { ok: true, count: data.length, data };
  ...
  async createTestScenarioForConstraintTest() {
    try {
      ...
      if (taskResponse.ok) {
        ...
        const deleteResponse = await this.utils.page.evaluate(
          async (clusterId) => {
            try {
              const resp = await fetch(
                `http://localhost:8000/api/v1/clusters/${clusterId}`,
  ...
  generateCRUDReport() {
    ...
    console.log(`\n🎯 关键发现:`);
    const deleteOps = this.results.operations.filter((op) =>
      op.operation.includes("删除"),
    );
    const createOps = this.results.operations.filter((op) =>
      op.operation.includes("创建"),
    );
    const readOps = this.results.operations.filter((op) =>
      op.operation.includes("读取"),
    );
    const updateOps = this.results.operations.filter((op) =>
      op.operation.includes("更新"),
    );
