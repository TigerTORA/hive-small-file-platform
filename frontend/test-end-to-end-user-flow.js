...
class EndToEndUserFlowTester {
  ...
  async testBackNavigation() {
    const backButton = await this.utils.elementExists(
      'button:has-text("返回")',
    );
    if (backButton) {
  ...
  async testClusterOperationButtons() {
    ...
    if (clusterCards.length > 0) {
      ...
      for (const button of buttons) {
        const buttonElement = await firstCard.$(
  ...
  async testTaskManagementFlow() {
    ...
    try {
      ...
      const hasCreateButton = await this.utils.elementExists(
