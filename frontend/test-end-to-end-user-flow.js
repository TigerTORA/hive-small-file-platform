...
class EndToEndUserFlowTester {
  ...
  async testNewUserExperienceFlow() {
    ...
    try {
      ...
      if (hasClusterCards && clusterCards.length > 0) {
        ...
        this.recordTestResult('cluster-list-display', true, `集群列表正常显示(${clusterCards.length}个)`);
  ...
  async testDataMonitoringFlow() {
    ...
    try {
      ...
      const hasTableData = await this.utils.elementExists('.table-container, .table-list, .el-table');
      if (hasTableData) {
  ...
  async testTaskManagementFlow() {
    ...
    try {
      ...
      // 验证任务管理页面元素
      const hasTaskContainer = await this.utils.elementExists('.task-container, .task-list, .el-table');
