...
def upgrade() -> None:
    ...
    op.add_column(
        "table_metrics",
        sa.Column("last_access_time", sa.DateTime(timezone=True), nullable=True),
    )
    op.add_column(
        "table_metrics",
        sa.Column("days_since_last_access", sa.Integer(), nullable=True),
    )
    op.add_column(
        "table_metrics",
        sa.Column("is_cold_data", sa.Integer(), nullable=True, default=0),
    )
    op.add_column(
        "table_metrics",
        sa.Column(
            "archive_status", sa.String(length=50), nullable=True, default="active"
        ),
    )
    op.add_column(
        "table_metrics",
        sa.Column("archive_location", sa.String(length=1000), nullable=True),
    )
    op.add_column(
        "table_metrics",
    ...
    op.create_index(
        op.f("ix_table_metrics_archive_status"),
        "table_metrics",
        ["archive_status"],
        unique=False,
    )
    op.create_index(
        op.f("ix_table_metrics_archived_at"),
        "table_metrics",
        ["archived_at"],
        unique=False,
    )
    op.create_index(
        op.f("ix_table_metrics_last_access_time"),
        "table_metrics",
        ["last_access_time"],
        unique=False,
    )
