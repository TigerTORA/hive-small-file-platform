...
class TableMetricFactory(SQLAlchemyModelFactory):
    ...
    table_type = factory.Faker("random_element", elements=("MANAGED_TABLE", "EXTERNAL_TABLE"))
    ...
    total_size = factory.Faker("random_int", min=1073741824, max=107374182400)  # 1GB-100GB
    small_files_size = factory.LazyAttribute(lambda obj: int(obj.total_size * 0.3))
    avg_file_size = factory.LazyAttribute(lambda obj: obj.total_size / obj.total_files if obj.total_files > 0 else 0)
...
class MergeTaskFactory(SQLAlchemyModelFactory):
    ...
    merge_strategy = factory.Faker("random_element", elements=["concatenate", "insert_overwrite"])
    target_file_size = factory.Faker("random_int", min=134217728, max=1073741824)  # 128MB to 1GB
    status = "pending"
    celery_task_id = None
    error_message = None
...
class TaskLogFactory(SQLAlchemyModelFactory):
    ...
    details = factory.Maybe(
        "has_details",
        yes_declaration=factory.Faker("json"),
        no_declaration=None
    )
...
def create_table_with_partitions(session, partitions_count: int = 3):
    ...
    for i in range(partitions_count):
        partition = PartitionMetricFactory(
            ...
            partition_spec=f"year=2024/month={i+1:02d}"
        )
