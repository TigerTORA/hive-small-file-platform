...
class TestClusterModel:
    ...
    def test_cluster_validation(self, db_session):
        ...
        with pytest.raises(ValueError):
            cluster = Cluster(
                name="test", hive_port=-1, hive_host="localhost"  # Invalid port
            )
    ...
    def test_cluster_metastore_url_format(self, db_session):
        ...
        # Valid MySQL URL
        cluster = ClusterFactory(metastore_url="mysql://user:pass@host:3306/hive")
        db_session.add(cluster)
        db_session.commit()
    ...
    def test_cluster_small_file_threshold(self, db_session):
        ...
        cluster2 = ClusterFactory(
            name="cluster-custom-threshold", small_file_threshold=67108864
        )
