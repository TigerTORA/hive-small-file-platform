...
class TestMockHDFSScanner:
    ...
    def test_scan_table_partitions_not_connected(self):
        ...
        with pytest.raises(ConnectionError, match="Not connected to HDFS"):
            self.scanner.scan_table_partitions(
                "/test/path", partitions, small_file_threshold
            )
    ...
    def test_scan_table_partitions_connected(self):
        ...
        assert (
            len(partition_stats) == 1
        )  # Non-partitioned table should have 1 default partition
    ...
    def test_mock_randomization(self):
        ...
        assert (
            len(unique_values) > 1 or len(results) == 1
        )  # Allow for small chance all are same
    ...
    def test_avg_file_size_calculation(self):
        ...
        if table_stats["total_files"] > 0:
            ...
            assert (
                abs(table_stats["avg_file_size"] - expected_avg) < 1.0
