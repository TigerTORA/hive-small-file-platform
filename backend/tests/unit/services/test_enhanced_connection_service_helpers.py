...
def test_classify_error_various_types():
    ...
    assert svc._classify_error(Exception("connection refused"), c) == FailureType.CONNECTION_REFUSED
    assert svc._classify_error(Exception("getaddrinfo failed"), c) == FailureType.DNS_RESOLUTION_FAILED
    assert svc._classify_error(Exception("authentication failed"), c) == FailureType.AUTHENTICATION_FAILED
    assert svc._classify_error(Exception("permission denied"), c) == FailureType.PERMISSION_DENIED
    assert svc._classify_error(Exception("ssl handshake"), c) == FailureType.SSL_ERROR
    assert svc._classify_error(Exception("service unavailable"), c) == FailureType.SERVICE_UNAVAILABLE
    assert svc._classify_error(Exception("totally unknown"), c) == FailureType.UNKNOWN_ERROR
...
def test_generate_failure_suggestions_contains_generic_and_specific():
    ...
    sugg = svc._generate_failure_suggestions(FailureType.NETWORK_TIMEOUT, ConnectionType.METASTORE)
...
def test_record_history_and_statistics():
    ...
    svc._record_connection_result(cid, ConnectionResult(
