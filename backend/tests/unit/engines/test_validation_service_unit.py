...
class _FakeMetastore:
    def __init__(self, is_partitioned=True, fmt=None):
        ...
        self._fmt = fmt or {"input_format": "org.apache.hadoop.mapred.TextInputFormat", "serde_lib": "org.apache.hadoop.hive.serde2.lazy.LazySimpleSerDe", "table_properties": {}}
    def __enter__(self):
        return self
    def __exit__(self, exc_type, exc, tb):
        return False
...
def test_validation_service_core_paths():
    svc = MergeTaskValidationService(_FakeConnMgr(table_exists=True, is_partitioned=True))
    task = MergeTask(cluster_id=1, task_name="t", database_name="db", table_name="tbl", merge_strategy="safe_merge")
    ...
    fmt_hudi = {"input_format": "", "serde_lib": "org.apache.hudi", "table_properties": {}}
    svc2 = MergeTaskValidationService(_FakeConnMgr(table_exists=True, is_partitioned=False, fmt=fmt_hudi))
