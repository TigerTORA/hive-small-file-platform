...
async def test_cold_summary_and_list(db_session):
    ...
    lst = await ta.get_cold_data_list(c.id, database_name=None, min_days_since_access=0, page=1, page_size=10, order_by="days_since_last_access", order_desc=True, db=db_session)
...
async def test_scan_cold_data_updates_flags(db_session, monkeypatch):
    ...
    class _Cold:
        ...
        def scan_cold_data(self, threshold_days: int, database_filter=None):
            return {"cold_tables": [{"database_name": "dbB", "table_name": "t3", "last_access_time": datetime.utcnow() - timedelta(days=20), "days_since_last_access": 20}]}

    monkeypatch.setattr(ta, "SimpleColdDataScanner", _Cold)
    res = await ta.scan_cold_data(c.id, cold_days_threshold=15, cold_threshold_days=None, database_name=None, db=db_session)
...
async def test_archive_and_restore_table(db_session, monkeypatch):
    ...
    m.archive_status = "archived"; m.archive_location = "/archive/dbC/t4"; db_session.commit()
    rs = await ta.restore_table(c.id, "dbC", "t4", db_session)
    assert rs["restore_result"]["restored"] is True

    # mark archived again and list
    m.archive_status = "archived"; m.archived_at = datetime.utcnow(); db_session.commit()
    lst = await ta.list_archived_tables(c.id, database_name=None, page=1, page_size=10, db=db_session)
