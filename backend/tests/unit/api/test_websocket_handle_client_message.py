...
async def test_handle_subscribe_unsubscribe_and_ping(monkeypatch):
    ...
    with (
        patch("app.api.websocket.websocket_manager.subscribe") as sub,
        patch("app.api.websocket.websocket_manager.unsubscribe") as unsub,
        patch(
            "app.api.websocket.websocket_manager._send_to_websocket",
        ...
        # ping
        await handle_client_message(
            ws, "u1", {"type": "ping", "data": {"timestamp": 123}}
        )
...
async def test_handle_get_status_and_unknown():
    ...
    with (
        patch(
            "app.api.websocket.websocket_manager.get_connection_stats",
            return_value={"ok": 1},
        ),
        patch(
            "app.api.websocket.websocket_manager._send_to_websocket",
