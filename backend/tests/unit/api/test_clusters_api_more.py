...
async def test_cluster_status_and_history_and_cache(db_session, monkeypatch):
    ...
    monkeypatch.setattr(clusters_api.cluster_status_service, "get_cluster_connection_summary", lambda db, cid: {"ok": True})
    ...
    class _Rec:
        ...
        class _D:
            def isoformat(self):
                return "t"
        created_at = _D()
    monkeypatch.setattr(clusters_api.cluster_status_service, "get_cluster_status_history", lambda db, cid, limit: [_Rec()])
    ...
    def fake_clear(cid):
        called["clr"] = True
    monkeypatch.setattr(clusters_api.cluster_status_service, "clear_connection_cache", fake_clear)
...
async def test_connection_history_and_statistics(db_session, monkeypatch):
    ...
    monkeypatch.setattr(clusters_api.enhanced_connection_service, "get_connection_history", lambda cid, limit: [{"status": "ok"}])
    rh = await clusters_api.get_connection_history(c.id, 10, db_session)
    assert rh["total_records"] == 1

    monkeypatch.setattr(clusters_api.enhanced_connection_service, "get_connection_statistics", lambda cid, hours: {"success_rate": 1.0})
...
async def test_connection_tests(db_session, monkeypatch):
    ...
    with pytest.raises(Exception):
        await clusters_api.test_cluster_connection_enhanced(c.id, ["bad"], False, db_session)

    # enhanced ok
    async def fake_test(db, cid, force_refresh, types=None):
        return {"overall_status": "ok", "tests": {}}
    monkeypatch.setattr(clusters_api.cluster_status_service, "test_cluster_connections", fake_test)
    eok = await clusters_api.test_cluster_connection_enhanced(c.id, ["metastore"], True, db_session)
    assert eok["test_mode"] == "enhanced" and eok["force_refresh"] is True

    # real mode
    rok = await clusters_api.test_cluster_connection(c.id, mode="real", force_refresh=False, db=db_session)
    ...
    class _Scanner:
        ...
        def test_connections(self):
            return {"overall_status": "success", "tests": {"metastore": {"status": "success"}, "hdfs": {"status": "success"}}}
    monkeypatch.setattr(clusters_api, "HybridTableScanner", _Scanner)
    mok = await clusters_api.test_cluster_connection(c.id, mode="mock", force_refresh=False, db=db_session)
...
async def test_update_cluster_status(db_session, monkeypatch):
    ...
    monkeypatch.setattr(clusters_api.cluster_status_service, "record_status_change", lambda db, cid, new_status, reason, message: _Rec())
    r = await clusters_api.update_cluster_status(c.id, new_status="inactive", db=db_session)
...
async def test_scan_all_cluster_tables_simple(db_session, monkeypatch):
    ...
    class _Scanner:
        ...
        def scan_database_tables(self, db, db_name, table_filter=None, max_tables=None):
            return {"status": "completed", "tables_scanned": 1, "total_files": 10, "total_small_files": 2, "scan_duration": 0.1}
    ...
    res = await clusters_api.scan_all_cluster_tables(c.id, max_tables_per_db=2, db=db_session)
