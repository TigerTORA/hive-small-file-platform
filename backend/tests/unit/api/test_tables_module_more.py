...
async def test_table_partitions_and_stats(db_session, monkeypatch):
    ...
    stats1 = await tables_mod.get_partition_file_stats(
        c.id, "db1", "t1", partition_spec=None, db=db_session
    )
    assert stats1["file_stats"]["total_files"] == 120

    stats2 = await tables_mod.get_partition_file_stats(
        c.id, "db1", "t1", partition_spec="year=2024", db=db_session
    )
...
async def test_test_connections_and_scan_all_databases(db_session, monkeypatch):
    ...
    class _Scanner:
        ...
        def scan_database_tables(
            self, db, database_name, table_filter=None, max_tables=None
        ):
...
async def test_cold_data_endpoints_and_scan_progress(db_session, monkeypatch):
    ...
    t = ScanTaskModel(
        cluster_id=c.id, task_type="cluster", task_name="t", status="running"
    )
    db_session.add(t)
    db_session.commit()
    ...
    # no in-memory task
    monkeypatch.setattr(
        scan_service_mod.scan_task_manager, "get_task", lambda task_id: None
    )
...
async def test_scan_database_tables_modes_and_single_table(db_session, monkeypatch):
    ...
    class _Scanner2:
        def __init__(self, cluster):
            self.hive_connector = SimpleNamespace(
                connect=lambda: True, disconnect=lambda: None
            )
            self.hdfs_scanner = SimpleNamespace(
                connect=lambda: True, disconnect=lambda: None
            )
    ...
    # mock mode
    r1 = await tables_mod.scan_database_tables(
        c.id, "dbA", table_filter=None, db=db_session
    )
    assert r1["status"] == "completed" and r1["scan_result"]["tables_scanned"] == 1

    # real mode
    r2 = await tables_mod.scan_database_tables_real(
        c.id, "dbA", table_filter=None, max_tables=3, strict_real=True, db=db_session
    )
...
async def test_table_info_and_partitions_list(db_session, monkeypatch):
    ...
    parts_list = await tables_mod.get_table_partitions_list(
        c.id, "dbA", "tB", db_session
    )
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_get_merge_preview_and_table_info_non_partitioned(
    ...
    prev = await tables_mod.get_merge_preview(
        c.id, "dbA", "tX", partition_filter=None, db=db_session
    )
