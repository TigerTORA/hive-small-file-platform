...
def test_merge_logger_flow_and_export(db_session):
    ...
    logger.log_file_statistics(MergePhase.FILE_ANALYSIS, "db1.t1", files_before=10, files_after=2)
    ...
    assert body["summary"]["total_entries"] == len(body["log_entries"]) or body["summary"]["total_entries"] >= 4
