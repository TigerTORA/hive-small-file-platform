...
def test_mysql_metastore():
    ...
    try:
        ...
        if connection_result["status"] == "success":
            ...
            with connector:
                ...
                if databases:
                    ...
                    for i, table in enumerate(tables[:10]):  # 只显示前10个表
                        logger.info(
                            f"  {i+1}. {table['table_name']} ({table['table_type']})"
                        )
                        logger.info(f"     路径: {table['table_path']}")
                        logger.info(
                            f"     分区表: {'是' if table['is_partitioned'] else '否'}"
                        )
                    ...
                    if partitioned_tables:
                        ...
                        partitions = connector.get_table_partitions(
                            test_db, test_table["table_name"]
                        )
            ...
            logger.error(
                f"❌ 连接失败: {connection_result.get('message', 'Unknown error')}"
            )
            return False
