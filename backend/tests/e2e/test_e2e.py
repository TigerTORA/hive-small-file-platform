...
def test_task_status_update(task_id: int):
    ...
    try:
        ...
        # 模拟任务执行流程
        statuses = [("running", "任务开始运行"), ("success", "任务执行成功")]
...
def test_engine_factory():
    ...
    try:
        ...
        for engine_type in engines.keys():
            ...
            if capabilities:
                print(
                    f"   {engine_type}: {capabilities.get('typical_performance', 'unknown')} 性能"
                )

        # 测试策略推荐
        recommendation = MergeEngineFactory.recommend_strategy(
            cluster=None, table_format="PARQUET", file_count=500  # 传入None，只测试逻辑
        )
...
def test_frontend_api_integration():
    ...
    try:
        ...
        frontend_api_path = os.path.join(
            os.path.dirname(__file__), "frontend", "src", "api", "tasks.ts"
        )
