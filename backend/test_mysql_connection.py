...
def test_mysql_connection():
    ...
    # MySQL 连接信息（从环境变量读取，避免硬编码敏感信息）
    # 优先级：HIVE_METASTORE_URL > MYSQL_URL > 示例本地连接（需自行调整）
    mysql_url = (
        os.getenv("HIVE_METASTORE_URL")
        or os.getenv("MYSQL_URL")
        or "mysql://root:password@localhost:3306/hive"
    )
        ...
        with MySQLHiveMetastoreConnector(mysql_url) as connector:
            ...
            if databases:
                ...
                for table in tables[:5]:
                    print(
                        f"  - {table['table_name']} ({table['table_type']}) "
