...
def demo_strategy_selection_tests():
    ...
    # 模拟引擎工厂的策略推荐逻辑
    def mock_recommend_strategy(table_format, file_count, table_size, partition_count=0, is_production=True):
        """模拟策略推荐逻辑"""
        # 小表且兼容格式 -> concatenate
        if table_size < 1024 * 1024 * 1024 and table_format in ['parquet', 'orc'] and file_count < 100:
            return "concatenate"
    ...
    for case in test_cases:
        try:
            ...
            if result["success"] == case["expected_success"]:
                ...
                print(f"  ❌ {case['name']}: {result['message']} (期望成功: {case['expected_success']})")
                failed += 1
        except Exception as e:
            print(f"  ❌ {case['name']}: 异常 - {e}")
            failed += 1
        ...
        try:
            ...
            if response["status_code"] == case["expected_status"]:
                if response["status_code"] == 200:
                    print(f"  ✅ {case['name']}: 任务创建成功, ID={response['data']['task']['id']}")
                else:
                    print(f"  ✅ {case['name']}: 正确返回错误状态 {response['status_code']}")
                passed += 1
            else:
                print(f"  ❌ {case['name']}: 状态码 {response['status_code']} (期望: {case['expected_status']})")
                failed += 1
        except Exception as e:
            print(f"  ❌ {case['name']}: 异常 - {e}")
            failed += 1
...
def demo_frontend_component_tests():
    ...
    class MockVueComponent:
        ...
        def find(self, selector):
            ...
            mock_elements = {
                ...
                '.cluster-card': [{'text': 'production-cluster'}, {'text': 'test-cluster'}],
                '.stat-card': [{'text': '总集群数'}, {'text': '在线集群'}, {'text': '离线集群'}],
    ...
    test_cases = [
        ...
        },
        {
            "name": "添加集群按钮存在",
            "component": "ClustersManagement",
            "test_func": lambda comp: comp.find('[data-test="add-cluster-btn"]')['exists']
        }
    ]
...
def demo_integration_tests():
    ...
    class MockIntegrationWorkflow:
        ...
        def acquire_lock(self, task_id):
            ...
            table_key = f"{task['cluster_id']}.{task['database_name']}.{task['table_name']}"
    ...
    test_scenarios = [
        {
            "name": "完整智能合并工作流程",
            "steps": [
                ("创建智能任务", lambda: workflow.create_smart_task(1, "test_db", "test_table")),
        ...
        },
        {
            "name": "并发锁冲突场景",
            "steps": [
                ("创建第一个任务", lambda: workflow.create_smart_task(1, "test_db", "concurrent_table")),
                ("创建第二个任务", lambda: workflow.create_smart_task(1, "test_db", "concurrent_table")),
                ("执行第一个任务", lambda task_id1, task_id2: workflow.execute_task(task_id1)),
                ("尝试执行第二个任务", lambda task_id1, task_id2: workflow.execute_task(task_id2))
            ]
        }
    ]
