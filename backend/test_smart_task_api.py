...
@pytest.mark.skip(reason="API endpoints need refactoring")
class TestSmartTaskCreationAPI:
    ...
    @patch("app.engines.engine_factory.MergeEngineFactory.create_smart_merge_task")
    def test_create_smart_task_success(self, mock_create_smart, mock_get_db):
        ...
        response = client.post("/api/v1/tasks/", json=self.smart_create_request)
    ...
    def test_create_smart_task_missing_required_fields(self, mock_get_db):
        ...
        # 缺少table_name字段
        invalid_request = {"cluster_id": 1, "database_name": "test_db"}

        response = client.post("/api/v1/tasks/", json=invalid_request)
    ...
    @patch("app.api.tasks.get_db")
    @patch("app.engines.engine_factory.MergeEngineFactory.create_smart_merge_task")
    def test_create_smart_task_without_partition_filter(
        self, mock_create_smart, mock_get_db
    ):
        """测试不带分区过滤器的智能任务创建"""
        mock_db = Mock()
        mock_get_db.return_value = mock_db
        ...
        response = client.post("/api/v1/tasks/", json=request_without_partition)
...
class TestTaskExecutionAPI:
    ...
    def test_cancel_task_success(self, mock_cancel_delay, mock_get_db):
        ...
        # 模拟取消任务返回
        mock_cancel_result = {"status": "success", "message": "Task 123 cancelled"}
