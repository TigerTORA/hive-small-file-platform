    ...
    try:
        ...
        for db_name in databases:
            ...
            for i in range(tables_in_db):
                ...
                total_size = (
                    total_files * avg_file_size
                    + random.randint(-1000, 1000) * 1024 * 1024
                )
        ...
        db.commit()
        print(
            f"✓ Created {total_tables} table metrics across {len(databases)} databases"
        )
        ...
        total_files = (
            ...
            .filter(TableMetric.cluster_id == 1)
            .with_entities(func.sum(TableMetric.total_files))
            .scalar()
            or 0
        )
        total_small_files = (
            db.query(TableMetric)
            .filter(TableMetric.cluster_id == 1)
            .with_entities(func.sum(TableMetric.small_files))
            .scalar()
            or 0
        )

        print(
            f"✓ Verification: {count} tables, {total_files} total files, {total_small_files} small files"
        )
