    ...
    try:
        ...
        try:
            result = await scan_all_cluster_tables(
                cluster_id=1, max_tables_per_db=3, max_databases=2, db=db
            )
            ...
            print(
                f"  Total Databases Scanned: {len(result.get('scan_results', {}).get('database_results', []))}"
            )
            ...
            database_results = result.get("scan_results", {}).get(
                "database_results", []
            )
