...
class MergeTaskLogger:
    ...
    def start_phase(
        self, phase: MergePhase, message: str, details: Dict[str, Any] = None
    ):
        """开始执行阶段"""
    ...
    def _default_code(
        self, phase: MergePhase, level: MergeLogLevel, message: str
    ) -> str:
        """为合并日志生成默认事件码（Mxxx）。"""
        base = {
    ...
    def log_hdfs_operation(
        ...
        self.log(
            phase=phase, level=level, message=message, details=details, hdfs_stats=stats
        )

    def log_yarn_monitoring(
        self,
        application_id: str,
        phase: MergePhase,
        progress: float = None,
        ...
        """记录YARN任务监控详情（包含队列与Tracking URL）"""
        queue = None
        tracking = None
        if isinstance(details, dict):
            queue = details.get("queue")
            tracking = details.get("tracking_url") or details.get(
        ...
        if details:
            yarn_details.update(
                {k: v for k, v in details.items() if k not in yarn_details}
            )
    ...
    def log_file_statistics(
        self,
        phase: MergePhase,
        table_name: str,
        files_before: int = None,
        files_after: int = None,
    ...
    def log_data_validation(
        ...
        self.log(phase=phase, level=level, message=message, details=validation_details)

    def log_task_completion(
        self, success: bool, total_duration_ms: int, final_stats: Dict[str, Any] = None
    ):
        """记录任务完成"""
        level = MergeLogLevel.INFO if success else MergeLogLevel.ERROR
    ...
    def export_logs_to_json(self) -> str:
        ...
        logs_data = {
            "task_info": {
                ...
                "table_name": self.task.table_name,
                "created_at": (
                    self.task.created_time.isoformat()
                    if self.task.created_time
                    else None
                ),
            },
    ...
    def _save_to_database(self, entry: MergeLogEntry):
        ...
        try:
            ...
            task_log = TaskLog(
                ...
                hdfs_stats=(
                    json.dumps(entry.hdfs_stats, ensure_ascii=False)
                    if entry.hdfs_stats
                    else None
                ),
