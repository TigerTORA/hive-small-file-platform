...
class PasswordEncryptor:
    ...
    def _get_fernet(cls) -> Fernet:
        ...
        if cls._fernet_instance is None:
            # 从环境变量或配置文件获取加密密钥
            from app.config.settings import settings  # 局部导入避免循环
            ...
            if not encryption_key:
                ...
                if env in {"development", "dev", "local", "test"}:
                    # 开发/测试环境：生成临时密钥，不输出明文
                    logger.warning(
                        "HIVE_PASSWORD_ENCRYPTION_KEY missing. Using ephemeral key for dev/test."
                    )
                    encryption_key = cls.generate_key()
                else:
                    # 生产环境必须提供密钥
                    raise RuntimeError(
                        "HIVE_PASSWORD_ENCRYPTION_KEY is required in production."
                    )
            ...
            try:
                ...
                if env in {"development", "dev", "local", "test"}:
                    ...
                    cls._fernet_instance = Fernet(encryption_key.encode())
                    logger.warning(
                        "Replaced with ephemeral dev key due to format error"
                    )
                else:
                    raise
    ...
    def decrypt_password(cls, encrypted_password: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(
                "Invalid encryption token - password may be corrupted or key changed"
            )
            return None
        except Exception as e:
