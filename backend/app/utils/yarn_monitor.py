...
class YarnResourceManagerMonitor:
    ...
    def __init__(self, resource_manager_urls: Union[str, List[str]], timeout: int = 30):
        ...
        # 清理URL格式
        self.resource_manager_urls = [url.rstrip('/') for url in self.resource_manager_urls]
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return YarnClusterInfo(
                    ...
                    ha_zookeeper_connection_string=cluster_info.get('haZookeeperConnectionString', ''),
                    hadoop_version=cluster_info.get('hadoopVersion', ''),
                    hadoop_build_version=cluster_info.get('hadoopBuildVersion', ''),
                    hadoop_version_built_on=cluster_info.get('hadoopVersionBuiltOn', ''),
    ...
    def get_applications(self, state: Optional[str] = None, user: Optional[str] = None,
                        application_types: Optional[List[str]] = None, limit: int = 100) -> List[YarnApplicationInfo]:
        """
        获取YARN应用列表
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for app in apps_data['app']:
                    applications.append(YarnApplicationInfo(
                        id=app.get('id', ''),
                        name=app.get('name', ''),
    ...
    def get_application_by_id(self, application_id: str) -> Optional[YarnApplicationInfo]:
        """
        根据应用ID获取应用信息
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Application {application_id} not found: HTTP {response.status_code}")
                return None
    ...
    def find_hive_applications(self, user: str, limit: int = 50) -> List[YarnApplicationInfo]:
        """
        查找用户的Hive相关应用
        ...
        applications = self.get_applications(
            user=user,
            application_types=["TEZ", "MAPREDUCE"],
            limit=limit
        )
        ...
        for app in applications:
            if 'hive' in app.name.lower() or 'insert' in app.name.lower() or 'select' in app.name.lower():
                hive_apps.append(app)
        
        logger.info(f"Found {len(hive_apps)} Hive applications")
        return hive_apps
    
    def monitor_application_progress(self, application_id: str, 
                                   check_interval: int = 5, max_wait: int = 3600) -> Dict:
        """
        监控应用执行进度
        ...
        while time.time() - start_time < max_wait:
            ...
            logger.info(f"Application {application_id} progress: {app_info.progress}%, "
                       f"state: {app_info.state}, final_status: {app_info.final_status}")
            
            # 检查是否完成
            if app_info.state in [YarnApplicationState.FINISHED.value, 
                                YarnApplicationState.FAILED.value, 
                                YarnApplicationState.KILLED.value]:
                logger.info(f"Application {application_id} completed with state: {app_info.state}")
                break
        ...
        return {
            ...
            "final_status": final_app_info.final_status if final_app_info else "UNKNOWN",
            "success": final_app_info.final_status == YarnApplicationFinalStatus.SUCCEEDED.value if final_app_info else False
        }
...
if __name__ == "__main__":
    ...
    if success:
        ...
        for app in apps[:3]:
            print(f"App: {app.id}, Name: {app.name}, State: {app.state}, Progress: {app.progress}%")
