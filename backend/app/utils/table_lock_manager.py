...
class TableLockManager:
        ...
        try:
            ...
            if existing_lock:
                ...
                if cls._is_lock_expired(existing_lock, lock_timeout_minutes):
                    logger.warning(
                        f"Found expired lock on table {database_name}.{table_name}, releasing it"
                    )
            ...
            logger.info(
                f"Acquired table lock for {database_name}.{table_name} by task {task_id}"
            )
            ...
            db.rollback()
            return {"success": False, "message": f"Failed to acquire lock: {str(e)}"}
    ...
    def release_table_lock(cls, db: Session, task_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if task.table_lock_acquired:
                ...
                logger.info(
                    f"Released table lock for {task.database_name}.{task.table_name} by task {task_id}"
                )
                ...
                return {"success": True, "message": "No lock was held by this task"}

        except Exception as e:
            logger.error(f"Failed to release table lock: {e}")
            db.rollback()
            return {"success": False, "message": f"Failed to release lock: {str(e)}"}

    @classmethod
    def check_table_lock_status(
        cls, db: Session, cluster_id: int, database_name: str, table_name: str
    ) -> Dict[str, Any]:
        """
        检查表锁状态
        ...
        try:
            ...
            if active_lock:
                return {
                    ...
                    "locked_since": active_lock.started_time
                    or active_lock.created_time,
                }
            else:
                return {
                    "locked": False,
    ...
    def cleanup_expired_locks(
        cls, db: Session, lock_timeout_minutes: int = 120
    ) -> Dict[str, Any]:
        """
        清理过期的锁
    ...
    def _force_release_lock(cls, db: Session, task: MergeTask):
        ...
        logger.warning(
            f"Force released expired lock for task {task.id} on table {task.database_name}.{task.table_name}"
        )

    @classmethod
    def get_all_table_locks(
        cls, db: Session, cluster_id: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        获取所有表锁信息
        ...
        """
        try:
            query = db.query(MergeTask).filter(MergeTask.table_lock_acquired == True)
