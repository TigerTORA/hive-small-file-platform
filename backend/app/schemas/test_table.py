...
class TestTableConfig(BaseModel):
    ...
    hdfs_base_path: str = Field(default="/user/test/small_files_test", description="HDFS基础路径(仅供参考,实际由Hive管理)")
    partition_count: int = Field(default=10, ge=1, le=1000, description="分区数量")
    files_per_partition: int = Field(default=100, ge=1, le=1000, description="每分区文件数")
    file_size_kb: int = Field(default=50, ge=1, le=1024, description="单文件大小(KB)")
    scenario: TestTableScenario = Field(default=TestTableScenario.DEFAULT, description="测试场景")

    @validator('partition_count', 'files_per_partition')
    def validate_file_counts(cls, v, values):
        """验证文件数量不会过大"""
        partition_count = values.get('partition_count', v if 'partition_count' in values else 10)
        files_per_partition = values.get('files_per_partition', v if 'files_per_partition' in values else 100)

        total_files = partition_count * files_per_partition
        if total_files > 50000:
            raise ValueError(f"总文件数 {total_files} 超过限制 (50000)，请减少分区数或每分区文件数")
...
class TestTableCreateRequest(BaseModel):
    ...
    force_recreate: bool = Field(default=False, description="是否强制重新创建（删除已存在的表）")
...
class TestTableTask(BaseModel):
    ...
    # 执行结果
    hdfs_files_created: Optional[int] = Field(default=None, description="创建的HDFS文件数")
    hive_partitions_added: Optional[int] = Field(default=None, description="添加的Hive分区数")
