            ...
            result.append(
                {
                    ...
                    "updated_time": (
                        c.updated_time.isoformat() if c.updated_time else None
                    ),
                }
            )
        return result
...
async def create_cluster(
    ...
    if existing:
        raise HTTPException(
            status_code=400, detail=f"Cluster name '{cluster.name}' already exists"
        )

    # 验证URL格式
    if not cluster.hive_metastore_url.startswith(
        ("mysql://", "postgresql://", "sqlite://")
    ):
        raise HTTPException(
            status_code=400,
            detail="Invalid MetaStore URL format. Must start with mysql://, postgresql://, or sqlite://",
        )

    if not cluster.hdfs_namenode_url.startswith(("hdfs://", "http://", "https://")):
        raise HTTPException(
            status_code=400,
            detail="Invalid HDFS URL format. Must start with hdfs://, http://, or https://",
        )
        ...
        try:
            ...
            if not hdfs_ok and hdfs_test.get("mode") != "mock":
                ...
                validation_results["warning"] = (
                    f"HDFS connection failed but cluster will be created: {hdfs_test.get('message', 'Unknown error')}"
                )
    ...
    try:
        ...
        if validation_results:
            response_data = {
                ...
                "created_at": (
                    db_cluster.created_at.isoformat() if db_cluster.created_at else None
                ),
                "updated_at": (
                    db_cluster.updated_at.isoformat() if db_cluster.updated_at else None
                ),
                "validation_results": validation_results,
            }
        ...
        db.rollback()
        raise HTTPException(
            status_code=400, detail=f"Failed to create cluster: {str(e)}"
        )
...
async def batch_health_check(
    ...
    try:
        ...
        successful_checks = sum(
            1 for r in results.values() if r.get("overall_status") != "error"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Batch health check failed: {str(e)}"
        )
...
async def get_cluster_stats(cluster_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        total_databases = (
            db.query(func.count(func.distinct(TableMetric.database_name)))
        ...
        # 统计总表数
        total_tables = (
            db.query(func.count(TableMetric.id))
            .filter(TableMetric.cluster_id == cluster_id)
            .scalar()
            or 0
        )
        ...
        small_file_tables = (
            db.query(func.count(TableMetric.id))
            .filter(TableMetric.cluster_id == cluster_id)
            .filter(TableMetric.small_files > 0)
            .scalar()
            or 0
        )
        ...
        # 统计小文件总数
        total_small_files = (
            db.query(func.sum(TableMetric.small_files))
            .filter(TableMetric.cluster_id == cluster_id)
            .scalar()
            or 0
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get cluster stats: {str(e)}"
        )
...
async def get_cluster_databases(cluster_id: int, db: Session = Depends(get_db)):
    ...
    try:
        # 从已扫描的表指标中获取数据库列表
        databases = (
            db.query(TableMetric.database_name.distinct())
            .filter(TableMetric.cluster_id == cluster_id)
            .all()
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get cluster databases: {str(e)}"
        )


@router.put("/{cluster_id}", response_model=ClusterResponse)
async def update_cluster(
    cluster_id: int, cluster_update: ClusterUpdate, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(
            status_code=400, detail=f"Failed to update cluster: {str(e)}"
        )
...
async def delete_cluster(cluster_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        scan_task_count = (
            int(scan_task_count_result[0]) if scan_task_count_result else 0
        )
        ...
        return {
            "message": f"Cluster '{cluster_name}' and {total_deleted} related records deleted successfully"
        }
    except Exception as e:
        db.rollback()
        raise HTTPException(
            status_code=400, detail=f"Failed to delete cluster: {str(e)}"
        )
    ...
    try:
        if mode == "real":
            ...
            mock_results = {
                ...
                "logs": [
                    {"level": "INFO", "message": "Mock test completed successfully"}
                ],
                "suggestions": [],
                "cached": False,
            }
...
async def get_cluster_test_connection(
    cluster_id: int, mode: str = "mock", db: Session = Depends(get_db)
):
...
async def test_connection_without_cluster(cluster: ClusterCreate):
    ...
    if not cluster.hive_metastore_url.startswith(
        ...
        return {
            ...
            "suggestions": [
                "MetaStore URL must start with mysql://, postgresql://, or sqlite://"
            ],
...
async def check_all_clusters_health(db: Session = Depends(get_db)):
    ...
    return {
        ...
        "healthy_count": len(
            [r for r in health_results if r["overall_health"] == "healthy"]
        ),
...
async def scan_all_cluster_tables(
    ...
    max_tables_per_db: int = Query(
        20, description="Maximum tables to scan per database"
    ),
        ...
        for db_name in databases:
            try:
                ...
                db_result = scanner.scan_database_tables(
                    db, db_name, table_filter=None, max_tables=max_tables_per_db
                )

                if db_result.get("status") == "completed":
                    scan_results["scanned_databases"] += 1
                    scan_results["total_tables_scanned"] += db_result.get(
                        "tables_scanned", 0
                    )
                    scan_results["total_files_found"] += db_result.get("total_files", 0)
                    scan_results["total_small_files"] += db_result.get(
                        "total_small_files", 0
                    )
                    ...
                    scan_results["errors"].append(
                        f"Database {db_name}: {db_result.get('error', 'Unknown error')}"
                    )
        ...
        scan_results.update(
            {
                ...
                "summary": {
                    ...
                    "databases_failed": len(databases)
                    - scan_results["scanned_databases"],
                    ...
                    "small_file_ratio": round(
                        (
                            (
                                scan_results["total_small_files"]
                                / scan_results["total_files_found"]
                                * 100
                            )
                            if scan_results["total_files_found"] > 0
                            else 0
                        ),
                        2,
                    ),
...
async def get_cluster_status(cluster_id: int, db: Session = Depends(get_db)):
    """获取集群状态信息"""
    try:
        status_info = cluster_status_service.get_cluster_connection_summary(
            db, cluster_id
        )
        return status_info
...
async def get_cluster_status_history(
    cluster_id: int, limit: int = Query(50, ge=1, le=100), db: Session = Depends(get_db)
):
    """获取集群状态变更历史"""
    cluster = db.query(Cluster).filter(Cluster.id == cluster_id).first()
    if not cluster:
    ...
    try:
        history = cluster_status_service.get_cluster_status_history(
            db, cluster_id, limit
        )
        return {
            "cluster_id": cluster_id,
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get status history: {str(e)}"
        )
...
async def update_cluster_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update cluster status: {str(e)}"
        )
...
async def get_connection_history(
    cluster_id: int, limit: int = Query(50, ge=1, le=200), db: Session = Depends(get_db)
):
    """获取集群连接历史记录"""
    cluster = db.query(Cluster).filter(Cluster.id == cluster_id).first()
    if not cluster:
...
async def get_connection_statistics(
    ...
    try:
        statistics = enhanced_connection_service.get_connection_statistics(
            cluster_id, hours
        )
        return {"cluster_id": cluster_id, "cluster_name": cluster.name, **statistics}
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to get connection statistics: {str(e)}"
        )
...
async def test_cluster_connection_enhanced(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Enhanced connection test failed: {str(e)}"
        )
