...
from app.schemas.table_metric import ScanRequest, TableMetricResponse
    ...
    try:
        ...
        if scan_request.table_name and scan_request.database_name:
            ...
            with MySQLHiveMetastoreConnector(cluster.hive_metastore_url) as connector:
                ...
                table_info = next(
                    (t for t in tables if t["table_name"] == scan_request.table_name),
                    None,
                )
                if not table_info:
                    raise HTTPException(status_code=404, detail="Table not found")

            table_result = scanner.scan_table(
                db, scan_request.database_name, table_info, strict_real=strict_real
            )
            return {
            ...
            result = scanner.scan_database_tables(
                db, scan_request.database_name, max_tables=None, strict_real=strict_real
            )
            return {
                "cluster_id": scan_request.cluster_id,
                "database_name": scan_request.database_name,
...
async def scan_all_cluster_databases_with_progress(
    ...
    max_tables_per_db: Optional[int] = Query(
        None, description="每库最大扫描表数（空表示不限制）"
    ),
    strict_real: bool = Query(True, description="严格实连模式（失败不降级Mock）"),
    db: Session = Depends(get_db),
):
    ...
    try:
        ...
        task_id = scan_task_manager.scan_cluster_with_progress(
            db, cluster_id, max_tables_per_db, strict_real
        )
...
async def scan_database_tables(
    ...
    try:
        ...
        result = scanner.scan_database_tables(
            db, database_name, table_filter, max_tables=10, strict_real=False
        )
        return {
            "cluster_id": cluster_id,
...
async def scan_database_tables_real(
    ...
    try:
        ...
        result = scanner.scan_database_tables(
            db, database_name, table_filter, limit, strict_real=strict_real
        )
        return {
            "cluster_id": cluster_id,
            "database_name": database_name,
    ...
    try:
        ...
        mock_result = {
            ...
            "scan_result": {
                ...
                "small_file_ratio": (
                    round(small_files / total_files * 100, 2) if total_files > 0 else 0
                ),
...
async def test_cluster_connections_real(cluster_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Real connection test failed: {str(e)}"
        )
...
async def get_partition_file_stats(
    ...
    try:
        ...
        # 获取文件统计
        if partition_spec:
            file_count = engine._get_file_count(
                database_name, table_name, partition_spec
            )
        ...
        return {
            ...
            "merge_recommendation": {
                ...
                "priority": (
                    ...
                    else "MEDIUM" if small_file_count > 50 else "LOW"
                ),
                "estimated_improvement": (
                    f"可减少约 {int(small_file_count * 0.8)} 个文件"
                    if file_count > 0
                    else "无需合并"
                ),
            },
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get partition stats: {str(e)}"
        )
...
async def get_table_info(
    ...
    try:
        ...
        storage_format = engine._infer_storage_format_name(fmt)
        compression = engine._infer_table_compression(fmt, storage_format)
        output_format = fmt.get("output_format")
        table_location = engine._get_table_location(database_name, table_name)
        ...
        table_info: Dict[str, Any] = {
            ...
            "input_format": fmt.get("input_format"),
            "output_format": output_format,
            "serde_lib": fmt.get("serde_lib"),
            "table_path": table_location,
        }
            ...
            table_info.update(
                {
                    ...
                    "storage_format": storage_format or latest_metric.storage_format,
                    "input_format": table_info.get("input_format")
                    or latest_metric.input_format,
                    "output_format": table_info.get("output_format")
                    or latest_metric.output_format,
                    "serde_lib": table_info.get("serde_lib")
                    or latest_metric.serde_lib,
                    "table_owner": latest_metric.table_owner,
                    "table_create_time": latest_metric.table_create_time.isoformat()
                    if latest_metric.table_create_time
                    else None,
                    "total_files": latest_metric.total_files,
                    "small_files": latest_metric.small_files,
                    "avg_file_size": latest_metric.avg_file_size,
                    "partition_columns": latest_metric.partition_columns,
                    "current_compression": latest_metric.current_compression,
                }
            )

        effective_compression = compression
        if effective_compression in (None, "", "DEFAULT"):
            effective_compression = (
                latest_metric.current_compression if latest_metric else None
            )
            if not effective_compression:
                if storage_format == "PARQUET":
                    effective_compression = "SNAPPY"
                elif storage_format == "ORC":
                    effective_compression = "ZLIB"
                elif storage_format in {"TEXTFILE", "TEXT"}:
                    effective_compression = "NONE"

        table_info["current_compression"] = effective_compression or "DEFAULT"
...
async def scan_all_databases(
    cluster_id: int,
    max_tables_per_db: int = Query(
        0, description="Maximum number of tables to scan per database (0 = no limit)"
    ),
    db: Session = Depends(get_db),
):
    ...
    try:
        ...
        for i, database_name in enumerate(databases, 1):
            ...
            try:
                ...
                # 扫描数据库
                limit = None if max_tables_per_db <= 0 else max_tables_per_db
                result = scanner.scan_database_tables(
                    db, database_name, table_filter=None, max_tables=limit
                )
                ...
                print(
                    f"✅ 数据库 {database_name} 扫描完成: {result['tables_scanned']}表, "
        ...
        return {
            ...
            "summary": {
                ...
                "small_file_ratio": round(
                    total_small_files / max(total_files_found, 1) * 100, 2
                ),
...
async def get_cold_data_summary(cluster_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get cold data summary: {str(e)}"
        )
...
async def get_cold_data_list(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get cold data list: {str(e)}"
        )


@router.get("/scan-progress/{task_id}", response_model=ScanTaskProgress)
async def get_scan_task_progress(task_id: str, db: Session = Depends(get_db)):
    """获取扫描任务的实时进度"""
    ...
    try:
        ...
        db_task = (
            db.query(ScanTaskModel).filter(ScanTaskModel.task_id == task_id).first()
        )
        if not db_task:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to read task progress: {str(e)}"
        )


@router.get("/scan-progress/cluster/{cluster_id}")
async def get_scan_progress(cluster_id: int, db: Session = Depends(get_db)):
    """获取扫描进度状态（简单实现）"""
    cluster = db.query(Cluster).filter(Cluster.id == cluster_id).first()
    if not cluster:
        raise HTTPException(status_code=404, detail="Cluster not found")
    ...
    for metric in metrics:
        ...
        if (
            database_stats[db_name]["last_scan"] is None
            or metric.scan_time > database_stats[db_name]["last_scan"]
        ):
...
async def get_archive_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get archive status: {str(e)}"
        )
...
async def list_archived_tables(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to list archived tables: {str(e)}"
        )


@router.get("/archive-statistics/{cluster_id}")
async def get_archive_statistics(cluster_id: int, db: Session = Depends(get_db)):
    """获取集群的归档统计信息"""
    cluster = db.query(Cluster).filter(Cluster.id == cluster_id).first()
    if not cluster:
        raise HTTPException(status_code=404, detail="Cluster not found")
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get archive statistics: {str(e)}"
        )
