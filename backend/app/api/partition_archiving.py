...
async def scan_cold_partitions(
    ...
    try:
            ...
            try:
                ...
                if not cluster_t:
                    scan_task_manager.error(
                        task.task_id, "E401", "集群不存在", db=db_thread, phase="scan"
                    )
                    ...
                    return
                cold_scanner = PartitionColdDataScanner(
                    cluster_t, cold_threshold_days or 90
                )
                scan_task_manager.info(
                    ...
                    phase="scan",
                    ctx={
                        "threshold_days": cold_threshold_days or 90,
                        "db": database_name,
                        "table": table_name,
        ...
        raise HTTPException(
            status_code=500, detail=f"Cold partition scan failed: {str(e)}"
        )
...
async def get_cold_partitions_summary(
    ...
    try:
        ...
        summary = cold_scanner.get_cold_partitions_summary(
            db_session=db, database_name=database_name, table_name=table_name
        )
        return summary

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to get cold partitions summary: {str(e)}"
        )
    ...
    try:
        ...
        if coldness_level == "very_cold":
            ...
            base_query = base_query.filter(
                PartitionMetric.days_since_last_access.between(90, 180)
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get cold partitions list: {str(e)}"
        )


@router.post(
    "/archive-partition/{cluster_id}/{database_name}/{table_name}/{partition_name}"
)
async def archive_partition(
    ...
    try:
        ...
        task = scan_task_manager.create_scan_task(
            ...
            cluster_id,
            "partition-archive",
            f"归档分区: {database_name}.{table_name}.{partition_name}",
        )
        ...
        def run_archive():
            ...
            try:
                ...
                res = engine.archive_partition(
                    db_thread, database_name, table_name, partition_name, force
                )
                scan_task_manager.info(
                    task.task_id,
                    "P190",
                    ...
                    phase="archive",
                    ctx={
                        "files_moved": res.get("files_moved"),
                        "archive_location": res.get("archive_location"),
                    },
                )
                scan_task_manager.complete_task(db_thread, task.task_id, success=True)
            except Exception as e:
...
@router.post(
    "/restore-partition/{cluster_id}/{database_name}/{table_name}/{partition_name}"
)
async def restore_partition(
    ...
    try:
        ...
        task = scan_task_manager.create_scan_task(
            ...
            cluster_id,
            "partition-restore",
            f"恢复分区: {database_name}.{table_name}.{partition_name}",
        )
        ...
        def run_restore():
            ...
            try:
                ...
                res = engine.restore_partition(
                    db_thread, database_name, table_name, partition_name
                )
                scan_task_manager.info(
                    task.task_id,
                    "R190",
                    ...
                    ctx={
                        ...
                        "restored_location": res.get("restored_location"),
                    },
                )
                scan_task_manager.complete_task(db_thread, task.task_id, success=True)
            except Exception as e:
    ...
    try:
        if not background:
            ...
            batch_result = archive_engine.batch_archive_partitions(
                db_session=db, partition_list=partition_list, force=force
            )
            return {
        ...
        task = scan_task_manager.create_scan_task(
            db,
            cluster_id,
            "batch-partition-archive",
            f"批量分区归档: {len(partition_list)} 个",
        )
        ...
        def run_batch():
            ...
            try:
                scan_task_manager.info(
                    ...
                    db=db_thread,
                    phase="archive",
                    ctx={"total": len(partition_list), "force": force},
                )
                cluster_t = (
                    db_thread.query(Cluster).filter(Cluster.id == cluster_id).first()
                )
                ...
                scan_task_manager.info(
                    ...
                    phase="archive",
                    ctx={
                        "success": res.get("success_count"),
                        "failed": res.get("failed_count"),
                    },
                )
                scan_task_manager.complete_task(db_thread, task.task_id, success=True)
            except Exception as e:
...
@router.get(
    "/partition-archive-status/{cluster_id}/{database_name}/{table_name}/{partition_name}"
)
async def get_partition_archive_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get partition status: {str(e)}"
        )
...
async def list_archived_partitions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to list archived partitions: {str(e)}"
        )
...
async def get_partition_archive_statistics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get partition archive statistics: {str(e)}",
        )
...
async def get_partition_coldness_distribution(
    ...
    try:
        ...
        # 统计不同冷热程度的分区数量
        very_cold_count = base_query.filter(
            PartitionMetric.days_since_last_access > 180
        ).count()
        cold_count = base_query.filter(
            PartitionMetric.days_since_last_access.between(90, 180)
        ).count()
        warm_count = base_query.filter(
            PartitionMetric.days_since_last_access < 90
        ).count()
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get partition coldness distribution: {str(e)}",
        )
    ...
    try:
        ...
        if not background:
            if partition_list:
                archive_engine = PartitionArchiveEngine(cluster)
                batch_result = archive_engine.batch_archive_partitions(
                    db_session=db, partition_list=partition_list, force=False
                )
                return {
            ...
            try:
                ...
                if dry_run or not partition_list:
                    scan_task_manager.info(
                        ...
                        db=db_thread,
                        phase="archive",
                        ctx={"candidates": len(partition_list)},
                    )
                    scan_task_manager.complete_task(
                        db_thread, task.task_id, success=True
                    )
                    return
        ...
        raise HTTPException(
            status_code=500, detail=f"Auto archive by policy failed: {str(e)}"
        )
