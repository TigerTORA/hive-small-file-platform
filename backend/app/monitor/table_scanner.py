...
class TableScanner:
    ...
    def scan_single_table(self, db_session: Session, database_name: str, table_name: str) -> Optional[Dict]:
        """
        扫描单个指定的表
        Args:
            db_session: 数据库会话
        ...
        try:
            ...
            logger.error(f"Failed to scan single table {database_name}.{table_name}: {e}")
            return None
