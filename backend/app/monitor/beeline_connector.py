...
class BeelineConnector:
    ...
    def __init__(self, host: str, port: int = 10000, username: str = None, password: str = None):
        self.host = host
        self.port = port
    ...
    def test_connection(self) -> Dict:
        ...
        try:
            ...
            if jdbc_test['success']:
                ...
                result['message'] = f"HiveServer2连接成功 (认证方式: {jdbc_test.get('auth_method', 'simple')})"
                ...
                result['status'] = 'failed'
                result['message'] = f"HiveServer2连接失败: {jdbc_test.get('error', '未知错误')}"
    ...
    def _test_port_connectivity(self) -> Dict:
        ...
        try:
            ...
            if result_code == 0:
                ...
                logger.info(f"端口连通性测试成功: {self.host}:{self.port} ({result['response_time_ms']}ms)")
            else:
        ...
        try:
            ...
            try:
                ...
                if process.returncode == 0:
                    result['success'] = True
                    result['auth_method'] = 'kerberos' if 'krb' in process.stdout.lower() else 'simple'
