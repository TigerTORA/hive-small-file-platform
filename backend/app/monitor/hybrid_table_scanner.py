...
class HybridTableScanner:
    def __init__(self, cluster: Any) -> None:
        ...
        self.hive_connector = self._create_metastore_connector(
    ...
    def test_connections(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logs.append(
                {"level": "INFO", "message": f"MetaStore: {meta_result.get('status')}"}
            )
    ...
    def scan_database_tables(
        ...
        for i, t in enumerate(tables, 1):
            ...
            try:
                ...
                if hdfs_ok and t.get("table_path"):
                    ...
                    try:
                        stat = self.hdfs_scanner.scan_directory(
                            t["table_path"],
                            small_file_threshold=getattr(
                                self.cluster, "small_file_threshold", None
                            )
                            or 128 * 1024 * 1024,
                        )  # type: ignore
                        ...
                        if files == 0:
                            ...
                            small / files > 0.8 if files > 0 else False
                        ):  # 小文件比例超过80%
                            errors.append(
                                f"表{table_name}: 小文件比例过高({small}/{files}, {small/files*100:.1f}%)"
                            )
                ...
                table_scan_time = time.time() - table_start_time
                error_msg = (
                    f"表{table_name}处理失败: {e} (耗时: {table_scan_time:.2f}秒)"
                )
    ...
    def scan_table(
        ...
        try:
            path = (
                table_info.get("table_path") if isinstance(table_info, dict) else None
            )
            if hdfs_ok and path:
                stat = self.hdfs_scanner.scan_directory(
                    path,
        ...
        return {
            ...
            "total_size": total_size,
            "is_partitioned": (
                bool(table_info.get("is_partitioned"))
                if isinstance(table_info, dict)
                else False
            ),
            "partition_count": (
                int(table_info.get("partition_count") or 0)
                if isinstance(table_info, dict)
                else 0
            ),
    ...
    def scan_table_with_coldness_check(
        self,
        db: Any,
        database_name: str,
        table_info: Dict[str, Any],
        ...
        # 2. 新增：冷数据检查
        try:
            coldness_info = self._check_table_coldness(
                database_name, table_info["table_name"], cold_threshold_days
            )
    ...
    def scan_single_table(
        ...
        with self.hive_connector as meta:
            tables = meta.get_tables(database_name)
            table_info = next(
                (t for t in tables if t.get("table_name") == table_name), None
            )
        ...
        metrics = {
            ...
            "is_partitioned": bool(table_info.get("is_partitioned")),
            "partition_count": int(table_info.get("partition_count") or 0),
        }

        # 执行扫描
        scan_result = self.scan_table(
            db, database_name, table_info, strict_real=strict_real
        )
        ...
        try:
            tm = TableMetric(
                ...
                small_files=scan_result.get("small_files", 0),
                total_size=scan_result.get("total_size", 0),
                avg_file_size=scan_result.get("avg_file_size", 0.0),
                is_partitioned=1 if metrics["is_partitioned"] else 0,
                partition_count=metrics["partition_count"],
                scan_time=datetime.utcnow(),
            )
    ...
    def _check_table_coldness(
        self, database_name: str, table_name: str, cold_threshold_days: int = 90
    ) -> Dict[str, Any]:
        """
        检查单个表的冷热程度
        Args:
        ...
        try:
            ...
            if hasattr(self.hive_connector, "get_table_last_access_time"):
                last_access_time = self.hive_connector.get_table_last_access_time(
            ...
            return {
                "last_access_time": (
                    last_access_time.isoformat() if last_access_time else None
                ),
    ...
    def scan_cold_data_for_cluster(
        self, db: Any, cold_threshold_days: int = 90
    ) -> Dict[str, Any]:
        """
        为整个集群执行冷数据扫描
        Args:
