...
class RealTableScanner:
    """
    真实版本的表扫描服务，连接实际的Hive MetaStore和HDFS
    """
    ...
    def __init__(self, cluster: Cluster):
        ...
        # 根据集群配置选择HDFS扫描器
        if cluster.hdfs_namenode_url.startswith('hdfs://nameservice'):
            # NameService高可用集群，使用WebHDFS
            self.hdfs_scanner = WebHDFSScanner(cluster.hdfs_namenode_url, cluster.hdfs_user)
        else:
            # 单点NameNode，也使用WebHDFS兼容性更好
            self.hdfs_scanner = WebHDFSScanner(cluster.hdfs_namenode_url, cluster.hdfs_user)
