...
class NativeHDFSScanner:
    ...
    def scan_path(self, path: str) -> Dict[str, any]:
        ...
        try:
            ...
            for line in ls_result.stdout.split('\n'):
                ...
                if len(parts) >= 8 and parts[0].startswith('-'):  # 确保是文件而不是目录
                    try:
                        ...
                        # 解析修改时间
                        try:
                            modification_time = int(datetime.strptime(
                                f"{date_part} {time_part}", "%Y-%m-%d %H:%M"
                            ).timestamp() * 1000)
                        except:
                            modification_time = int(time.time() * 1000)  # 使用当前时间作为后备
            ...
            logger.info(f"扫描完成: {len(files)}个文件, {small_files_count}个小文件, 耗时{scan_duration:.2f}秒")
