...
class BaseMetastoreConnector(ABC):
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Connected to {self.__class__.__name__}: {self.parsed_url.hostname}"
            )
            return True
    ...
    def test_connection(self) -> Dict[str, any]:
        ...
        try:
            ...
            # 获取基本的MetaStore信息
            table_count_results = self._execute_query(
                "SELECT COUNT(*) as table_count FROM TBLS"
            )
            table_count = (
                table_count_results[0]["table_count"]
                if isinstance(table_count_results[0], dict)
                else table_count_results[0][0]
            )

            db_count_results = self._execute_query(
                "SELECT COUNT(*) as db_count FROM DBS"
            )
            db_count = (
                db_count_results[0]["db_count"]
                if isinstance(db_count_results[0], dict)
                else db_count_results[0][0]
            )
