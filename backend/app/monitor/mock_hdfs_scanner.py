...
class MockHDFSScanner:
    ...
    def scan_table_partitions(
        ...
        total_size = random.randint(
            1024 * 1024 * 100, 1024 * 1024 * 1024 * 50
        )  # 100MB to 50GB
        ...
        if partitions:
            ...
            for partition in partitions:
                partition_files = random.randint(
                    10, int(total_files / len(partitions)) * 2
                )
                partition_small_files = random.randint(0, partition_files)
                partition_size = random.randint(
                    1024 * 1024 * 10, total_size // len(partitions) * 2
                )
                ...
                partition_stats.append(
                    {
                        ...
                        "avg_file_size": (
                            partition_size / partition_files
