...
class PartitionArchiveEngine:
    ...
    def archive_partition(self, db_session: Session,
                        database_name: str, table_name: str, partition_name: str,
        ...
        try:
            ...
            if not partition_metric:
                raise ValueError(f"分区 {database_name}.{table_name}.{partition_name} 在系统中不存在")

            # 2. 检查分区状态
            if not force and partition_metric.archive_status != 'active':
                raise ValueError(f"分区已被归档，状态: {partition_metric.archive_status}")

            if not force and partition_metric.is_cold_data != 1:
                raise ValueError(f"分区不是冷数据，无法归档")

            # 3. 获取分区的HDFS路径信息
            partition_location = self._get_partition_location(database_name, table_name, partition_name)
            if not partition_location:
                raise ValueError(f"无法获取分区存储位置")

            # 4. 创建归档目录结构
            archive_path = self._create_partition_archive_path(database_name, table_name, partition_name)
            ...
            logger.info(f"分区 {database_name}.{table_name}.{partition_name} 归档成功，移动了 {len(moved_files)} 个文件")
            return result

        except Exception as e:
            logger.error(f"归档分区 {database_name}.{table_name}.{partition_name} 失败: {e}")
            db_session.rollback()
            raise

    def restore_partition(self, db_session: Session,
                        database_name: str, table_name: str, partition_name: str) -> Dict:
        """
        恢复归档分区
        Args:
            db_session: 数据库会话
        ...
        try:
            ...
            if partition_metric.archive_status != 'archived':
                raise ValueError(f"分区未被归档，状态: {partition_metric.archive_status}")

            if not partition_metric.archive_location:
                raise ValueError(f"分区缺少归档位置信息")

            # 3. 获取原始分区位置
            original_location = self._get_partition_location(database_name, table_name, partition_name)
            if not original_location:
                raise ValueError(f"无法获取分区原始存储位置")

            # 4. 执行数据文件恢复
            restored_files = self._restore_partition_data(partition_metric.archive_location, original_location)
            ...
            logger.info(f"分区 {database_name}.{table_name}.{partition_name} 恢复成功，恢复了 {len(restored_files)} 个文件")
            return result

        except Exception as e:
            logger.error(f"恢复分区 {database_name}.{table_name}.{partition_name} 失败: {e}")
            db_session.rollback()
            raise

    def batch_archive_partitions(self, db_session: Session,
        ...
        for partition_info in partition_list:
            try:
                ...
                failed_count += 1
                logger.error(f"批量归档中处理分区失败: {error_result['partition_full_name']}, 错误: {e}")
    ...
    def get_partition_archive_status(self, db_session: Session,
                                   database_name: str, table_name: str, partition_name: str) -> Dict:
        """
        获取分区的归档状态信息
        Args:
        ...
        Returns:
            归档状态字典
        """
        partition_metric = self._get_partition_metric(db_session, database_name, table_name, partition_name)
        if not partition_metric:
            return {
                'exists': False,
                'partition_full_name': f"{database_name}.{table_name}.{partition_name}"
        ...
        return {
            ...
            'archive_location': partition_metric.archive_location,
            'archived_at': partition_metric.archived_at.isoformat() if partition_metric.archived_at else None,
            'is_cold_data': bool(partition_metric.is_cold_data),
            'days_since_last_access': partition_metric.days_since_last_access,
            'last_access_time': partition_metric.last_access_time.isoformat() if partition_metric.last_access_time else None,
            'total_size': partition_metric.total_size,
            'file_count': partition_metric.file_count
        }

    def list_archived_partitions(self, db_session: Session,
        ...
        try:
            ...
            archived_partitions = base_query.order_by(
                PartitionMetric.archived_at.desc()
            ).limit(limit).all()
            ...
            for partition in archived_partitions:
                ...
                partition_info = {
                    ...
                    'archived_at': partition.archived_at.isoformat() if partition.archived_at else None,
                    'days_since_last_access': partition.days_since_last_access,
                    'last_access_time': partition.last_access_time.isoformat() if partition.last_access_time else None,
    ...
    def _get_partition_metric(self, db_session: Session,
                            database_name: str, table_name: str, partition_name: str) -> Optional[PartitionMetric]:
        """获取分区指标记录"""
        ...
        return db_session.query(PartitionMetric).filter(
            PartitionMetric.table_metric_id == table_metric.id,
            PartitionMetric.partition_name == partition_name
        ).first()

    def _get_partition_location(self, database_name: str, table_name: str, partition_name: str) -> Optional[str]:
        """
        从MetaStore获取分区的存储位置
        Args:
            database_name: 数据库名
        ...
        try:
            with MySQLHiveMetastoreConnector(self.cluster.hive_metastore_url) as conn:
                ...
                logger.warning(f"未找到分区 {database_name}.{table_name}.{partition_name} 的位置信息")
                return None
        except Exception as e:
            logger.error(f"获取分区 {database_name}.{table_name}.{partition_name} 存储位置失败: {e}")
            return None

    def _create_partition_archive_path(self, database_name: str, table_name: str, partition_name: str) -> str:
        """
        创建分区归档路径
        Args:
            database_name: 数据库名
            ...
            try:
                ...
                for file_info in archived_files:
                    if not file_info.is_directory:
                        ...
                        original_file = os.path.join(source_path, relative_path).replace('\\', '/')
                ...
                logger.info(f"成功移动 {len(moved_files)} 个文件从 {source_path} 到 {target_path}")
                return moved_files
    ...
    def _restore_partition_data(self, archive_path: str, restore_path: str) -> List[str]:
        """
        从归档位置恢复分区数据文件
        Args:
            archive_path: 归档路径
            ...
            try:
                ...
                for file_info in restored_files_info:
                    if not file_info.is_directory:
                        ...
                        archive_file = os.path.join(archive_path, relative_path).replace('\\', '/')
                ...
                logger.info(f"成功恢复 {len(restored_files)} 个文件从 {archive_path} 到 {restore_path}")
    ...
    def get_partition_archive_statistics(self, db_session: Session) -> Dict:
        ...
        try:
            # 统计各状态的分区数量
            total_partitions = db_session.query(PartitionMetric).join(TableMetric).filter(
                TableMetric.cluster_id == self.cluster.id
            ).count()

            archived_partitions = db_session.query(PartitionMetric).join(TableMetric).filter(
                TableMetric.cluster_id == self.cluster.id,
                PartitionMetric.archive_status == 'archived'
            ).count()

            active_partitions = db_session.query(PartitionMetric).join(TableMetric).filter(
                TableMetric.cluster_id == self.cluster.id,
            ...
            archived_size = db_session.query(
                db_session.query(func.sum(PartitionMetric.total_size)).join(TableMetric).filter(
                    TableMetric.cluster_id == self.cluster.id,
                    PartitionMetric.archive_status == 'archived'
                ).scalar_subquery()
            ).scalar() or 0
            ...
            # 最近归档的分区
            recent_archived = db_session.query(PartitionMetric).join(TableMetric).filter(
                TableMetric.cluster_id == self.cluster.id,
                PartitionMetric.archive_status == 'archived'
            ).order_by(
                PartitionMetric.archived_at.desc()
            ...
            return {
                ...
                'statistics': {
                    ...
                    'archive_ratio': round(archived_partitions / max(total_partitions, 1) * 100, 2),
