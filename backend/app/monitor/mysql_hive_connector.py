...
class MySQLHiveMetastoreConnector(BaseMetastoreConnector):
    ...
    def get_table_partitions_count(self, database_name: str, table_name: str) -> int:
        ...
        try:
            ...
            logger.error(
                f"Failed to count partitions for {database_name}.{table_name}: {e}"
            )
            return 0

    def get_table_partitions_paged(
        self, database_name: str, table_name: str, offset: int, limit: int
    ) -> List[Dict]:
        """分页获取表分区信息（避免一次性全量查询）"""
        if not self._connection:
            raise ConnectionError("Not connected to MetaStore")
        try:
            ...
            logger.error(
                f"Failed to get paged partitions for {database_name}.{table_name}: {e}"
            )
            return []
    ...
    def get_table_access_info(
        self, database_name: Optional[str] = None, table_name: Optional[str] = None
    ) -> List[Dict]:
        """
        获取表的访问时间信息
        Args:
                ...
                return [
                    {
                        ...
                        "last_access_time": self._timestamp_to_datetime(
    ...
    def get_table_last_access_time(
        ...
        try:
            ...
            logger.error(
                f"Failed to get last access time for {database_name}.{table_name}: {e}"
            )
            return None
    ...
    def get_partition_access_info(
        ...
        try:
            with self._connection.cursor() as cursor:
                ...
                logger.info(
                    f"Retrieved access info for {len(partition_list)} partitions"
                )
