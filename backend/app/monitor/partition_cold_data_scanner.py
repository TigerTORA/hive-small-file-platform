...
class PartitionColdDataScanner:
    ...
    def scan_cold_partitions(
        ...
        """
        logger.info(
            f"开始扫描集群 {self.cluster.name} 的分区级冷数据，阈值: {self.cold_days_threshold}天"
        )

        try:
            # 1. 从MetaStore获取分区访问时间信息
            with MySQLHiveMetastoreConnector(self.cluster.hive_metastore_url) as conn:
                partition_access_info = conn.get_partition_access_info(
                    database_name, table_name
                )

            if not partition_access_info:
                logger.warning(
                    f"未获取到数据库 {database_name or 'ALL'} 的分区访问信息"
                )
                return {
            ...
            for info in partition_access_info:
                try:
                    ...
                    if is_cold:
                        ...
                        logger.debug(
                            f"标记冷数据分区: {info['database_name']}.{info['table_name']}.{info['partition_name']} "
                            f"(距离上次访问: {days_since_access}天，大小: {partition_size}字节)"
                        )

                except Exception as e:
                    logger.error(
                        f"处理分区 {info.get('database_name')}.{info.get('table_name')}.{info.get('partition_name')} 时出错: {e}"
                    )
                    continue
            ...
            logger.info(
                f"分区冷数据扫描完成: 扫描{len(partition_access_info)}个分区，发现{len(cold_partitions)}个冷数据分区"
            )
    ...
    def _calculate_days_since_access(self, info: Dict, current_time: datetime) -> int:
        ...
        if not last_access:
            ...
            if create_time:
                ...
                logger.warning(
                    f"分区 {info['database_name']}.{info['table_name']}.{info['partition_name']} 缺少访问时间和创建时间"
                )
                return 999  # 无任何时间信息，标记为很久

        return (current_time - last_access).days

    def _get_or_create_partition_metric(
        self,
        db_session: Session,
        database_name: str,
        table_name: str,
    ...
    def get_cold_partitions_summary(
        self,
        db_session: Session,
        database_name: Optional[str] = None,
        table_name: Optional[str] = None,
        ...
        try:
            ...
            # 统计冷分区
            cold_partition_count = base_query.filter(
                PartitionMetric.is_cold_data == 1
            ).count()
            total_partition_count = base_query.count()

            # 统计不同冷度区间的分区数量
            very_cold = base_query.filter(
                PartitionMetric.days_since_last_access > 180
            ).count()  # 6个月以上
            cold = base_query.filter(
                PartitionMetric.days_since_last_access.between(90, 180)
            ).count()  # 3-6个月

            # 获取冷分区总大小
            cold_partitions_size = (
                base_query.filter(PartitionMetric.is_cold_data == 1)
                .with_entities(
                    db_session.query(
                        func.sum(PartitionMetric.total_size)
                    ).scalar_subquery()
                )
                .scalar()
                or 0
            ...
            cold_partitions = (
                base_query.filter(PartitionMetric.is_cold_data == 1)
                .order_by(PartitionMetric.days_since_last_access.desc())
                .limit(20)
                .all()
            )
            ...
            return {
                ...
                "cold_ratio": round(
                    cold_partition_count / max(total_partition_count, 1) * 100, 2
                ),
                "threshold_days": self.cold_days_threshold,
                "distribution": {
    ...
    def get_partitions_by_coldness(
        self,
        db_session: Session,
        ...
        partitions = (
            base_query.order_by(PartitionMetric.days_since_last_access.desc())
            .limit(limit)
            .all()
        )
            ...
            partition_list.append(
                {
                    ...
                    "days_since_access": p.days_since_last_access,
                    "last_access_time": (
                        p.last_access_time.isoformat() if p.last_access_time else None
                    ),
                    "total_size": p.total_size,
                    "file_count": p.file_count,
