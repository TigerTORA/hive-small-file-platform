...
class PathResolver:
    @staticmethod
    def _resolve_via_metastore(metastore_url: str, database_name: str, table_name: str) -> Optional[str]:
        try:
    ...
    def _resolve_via_hs2(cluster, database_name: str, table_name: str) -> Optional[str]:
        ...
        try:
            ...
            params = {
                ...
                'database': database_name or getattr(cluster, 'hive_database', 'default') or 'default',
            }
            if (getattr(cluster, 'auth_type', '') or '').upper() == 'LDAP' and getattr(cluster, 'hive_username', None):
            ...
            for row in rows:
                if len(row) >= 2 and row[0] and ('Location:' in str(row[0]) or str(row[0]).strip().lower() == 'location'):
    ...
    def get_table_location(cluster, database_name: str, table_name: str) -> str:
        # 1) Try MetaStore
        loc = PathResolver._resolve_via_metastore(cluster.hive_metastore_url, database_name, table_name)
        if loc:
            return loc
    ...
    @staticmethod
    def get_partition_locations(cluster, database_name: str, table_name: str) -> List[str]:
        ...
        try:
            ...
            with connector as conn:
                ...
                locs = [p.get('partition_path') for p in parts if p.get('partition_path')]
            if locs:
                paths.extend(locs)
        except Exception as e:
