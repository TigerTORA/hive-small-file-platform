...
class HivePartitionMergeExecutor:
    ...
    def execute_single_partition_native_merge(
        ...
        original_file_count = self.engine.file_counter.count_partition_files(
            original_partition_path
        )
        merge_logger.log(
            MergePhase.EXECUTION,
            MergeLogLevel.INFO,
        ...
        # 3. 生成临时分区名称
        ts = int(time.time())
        temp_partition_kv = self.path_resolver.generate_temp_partition_kv(
            partition_kv, ts
        )
        temp_partition_spec = self.path_resolver.format_partition_spec(
            temp_partition_kv
        )
        ...
        try:
            ...
            self.engine._wait_for_partition_data(
            ...
            merged_file_count = self.engine.file_counter.count_partition_files(
                merged_partition_path
            )
    ...
    def execute_full_table_dynamic_partition_merge(
        ...
        try:
            # 1. 获取分区列
            partition_cols = self.engine.metadata_manager._get_partition_columns(
                database, table
            )
            ...
            files_before = None
            try:
                table_location = self.engine.metadata_manager._get_table_location(
                    database, table
                )
                if table_location:
            ...
            # 解析原表结构(不继承ACID属性)
            schema_info = (
                self.engine.metadata_manager._parse_table_schema_from_show_create(
                    database, table
                )
            )
            ...
            if task.target_storage_format:
                ...
                input_fmt, output_fmt, serde = self.engine._get_format_classes(
                    target_format
                )
                merge_logger.log(
                    MergePhase.TEMP_TABLE_CREATION,
