...
class RealHiveMergeEngine(BaseMergeEngine):
    ...
    def validate_task(self, task: MergeTask) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"RealHive validation completed for {task.database_name}.{task.table_name}")
    ...
    def execute_merge(self, task: MergeTask, db_session: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            merge_logger.start_phase(MergePhase.INITIALIZATION, "初始化真实Hive合并任务", {
            ...
            self._report_progress('initializing', 'Starting real Hive merge with impyla connection')
            
            # 建立真实的Hive连接
            merge_logger.start_phase(MergePhase.CONNECTION_TEST, "建立真实Hive、HDFS、YARN连接")
            self._report_progress('connecting', 'Establishing real connections to Hive and HDFS')
            ...
            merge_logger.end_phase(MergePhase.CONNECTION_TEST, "所有真实连接测试成功", success=True)
            ...
            files_before = self._get_real_file_count_with_logging(task.database_name, task.table_name, 
            ...
            merge_logger.end_phase(MergePhase.FILE_ANALYSIS, f"真实文件分析完成，当前{files_before}个文件")
            ...
            # 第一步：创建临时表并执行真实合并
            merge_logger.start_phase(MergePhase.TEMP_TABLE_CREATION, f"创建真实临时表: {temp_table_name}")
            self._report_progress('creating_temp_table', f'Creating real temporary table: {temp_table_name}')
            ...
            create_temp_sql = self._create_real_temp_table_with_logging(task, temp_table_name, merge_logger)
            result['sql_executed'].extend(create_temp_sql)
            result['temp_table_created'] = temp_table_name
            
            merge_logger.end_phase(MergePhase.TEMP_TABLE_CREATION, f"真实临时表创建完成: {temp_table_name}")
            ...
            # 第二步：验证临时表数据
            merge_logger.start_phase(MergePhase.DATA_VALIDATION, "验证真实临时表数据完整性")
            self._report_progress('validating', 'Validating real temporary table data integrity')
            ...
            validation_result = self._validate_real_temp_table_data(task, temp_table_name)
            ...
            # 获取合并后的文件统计（从临时表）
            files_after = self._get_real_temp_table_file_count(task.database_name, temp_table_name, task.partition_filter)
            ...
            swap_sql = self._real_atomic_table_swap_with_logging(task, temp_table_name, backup_table_name, merge_logger)
            ...
            result['message'] = f'真实Hive合并成功完成。文件从{files_before}个减少到{files_after}个'
            ...
            self._report_progress('completed', f'Real Hive merge completed successfully. Files: {files_before} → {files_after}')
            ...
            self.log_task_event(task, 'INFO', f'真实备份表已创建: {backup_table_name}. 验证后可手动删除.', db_session=db_session)
            ...
            # 执行回滚操作
            try:
                self._report_progress('rolling_back', 'Starting rollback process to clean up real temporary resources')
                self.log_task_event(task, 'WARNING', '开始回滚真实资源', db_session=db_session)
                rollback_sql = self._rollback_real_merge(task, temp_table_name, backup_table_name)
                result['sql_executed'].extend(rollback_sql)
                self.log_task_event(task, 'INFO', '回滚成功完成', db_session=db_session)
            except Exception as rollback_error:
                self.log_task_event(task, 'ERROR', f'回滚失败: {rollback_error}', db_session=db_session)
    ...
    def _test_connections(self) -> bool:
        ...
        try:
            ...
            if not hdfs_ok:
                logger.warning(f"WebHDFS connection failed: {hdfs_msg} (non-blocking)")
        except Exception as e:
            logger.warning(f"WebHDFS connection exception: {e} (non-blocking)")
    ...
    def _create_real_hive_connection(self, database_name: str = None):
        ...
        if (self.cluster.auth_type or '').upper() == 'LDAP' and self.cluster.hive_username:
            candidates.extend([
                {'auth_mechanism': 'PLAIN', 'use_ssl': False, 'user': self.cluster.hive_username, 'password': self.hive_password},
                {'auth_mechanism': 'PLAIN', 'use_ssl': True,  'user': self.cluster.hive_username, 'password': self.hive_password},
            ])
        else:
            candidates.extend([
        ...
        for c in candidates:
            try:
                params = {'host': host, 'port': port, 'database': db, 'timeout': 10, 'auth_mechanism': c['auth_mechanism']}
                ...
                if c.get('use_ssl'):
                    params['use_ssl'] = True
                logger.info(f"Attempting Hive connection host={host} port={port} auth={params['auth_mechanism']} ssl={c.get('use_ssl', False)}")
    ...
    def _get_real_file_count_with_logging(self, database_name: str, table_name: str, 
        ...
        try:
            ...
            if stats.get('success', False):
                ...
                logger.info(f"Real file count for {database_name}.{table_name}: {file_count}")
                return file_count
            else:
                merge_logger.log_hdfs_operation(
                    "get_real_table_stats", table_location, MergePhase.FILE_ANALYSIS,
    ...
    def _create_real_temp_table_with_logging(self, task: MergeTask, temp_table_name: str, merge_logger) -> List[str]:
        ...
        try:
            ...
            logger.info(f"Real SQL executed: {create_sql}")
    ...
    def _validate_real_temp_table_data(self, task: MergeTask, temp_table_name: str) -> Dict[str, Any]:
        """验证临时表数据完整性（真实查询）"""
        result = {
            'valid': True,
    ...
    def _get_real_temp_table_file_count(self, database_name: str, temp_table_name: str, partition_filter: Optional[str] = None) -> int:
        """获取临时表的文件数量（真实查询）"""
        try:
            # 获取临时表的HDFS路径（真实查询）
            table_location = self._get_table_location(database_name, temp_table_name)
            if not table_location:
                logger.error(f"Could not get real temp table location for {database_name}.{temp_table_name}")
                return 0
            
            logger.info(f"Getting real file count for temp table {database_name}.{temp_table_name} at location: {table_location}")
            ...
            if stats.get('success', False):
                ...
                logger.info(f"Real WebHDFS stats for temp table {database_name}.{temp_table_name}: {total_files} files")
                return total_files
            else:
                logger.error(f"Real WebHDFS stats failed for temp table: {stats.get('error', 'Unknown error')}")
                return 0
    ...
    def _real_atomic_table_swap_with_logging(self, task: MergeTask, temp_table_name: str, 
    ...
    def _rollback_real_merge(self, task: MergeTask, temp_table_name: str, backup_table_name: str) -> List[str]:
        """回滚真实合并操作（真实SQL执行）"""
        sql_statements = []
    ...
    def get_merge_preview(self, task: MergeTask) -> Dict[str, Any]:
        ...
        try:
            ...
            # 获取当前文件数量（真实查询）
            current_files = self._get_real_file_count_with_logging(task.database_name, task.table_name, task.partition_filter, MergeTaskLogger(task, None))
    ...
    def estimate_duration(self, task: MergeTask) -> int:
        ...
        try:
            file_count = self._get_real_file_count_with_logging(task.database_name, task.table_name, task.partition_filter, MergeTaskLogger(task, None))
