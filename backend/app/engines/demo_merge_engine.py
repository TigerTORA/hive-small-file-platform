...
class DemoMergeEngine:
    ...
    def __init__(self, cluster: Cluster):
        ...
        self.webhdfs_client = WebHDFSClient(cluster.hdfs_namenode_url, cluster.hdfs_user)
        self.metastore_connector = HiveMetastoreConnector(cluster.hive_metastore_url)
        
        # YARN监控（如果配置了）
        if cluster.yarn_resource_manager_url:
            self.yarn_monitor = YarnResourceManagerMonitor(cluster.yarn_resource_manager_url)
        else:
    ...
    def execute_merge(self, task: MergeTask, db_session: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            self._update_task_progress(task, db_session, "initialization", 5.0, "初始化合并任务")
            ...
            self._update_task_progress(task, db_session, "connection_test", 10.0, "测试连接中")
            ...
            # 记录连接测试详情
            merge_logger.log(
                MergePhase.CONNECTION_TEST, MergeLogLevel.INFO, 
                "MetaStore连接测试成功",
                details={"connector_type": "mysql", "database": task.database_name}
            )
            
            merge_logger.log_hdfs_operation(
                "连接测试", "/", MergePhase.CONNECTION_TEST,
                stats={"namenode_available": True}, success=True
            )
            
            if self.yarn_monitor:
                merge_logger.log_yarn_monitoring(
                    "demo_application_123", MergePhase.CONNECTION_TEST,
                    state="CONNECTING", details={"cluster_status": "healthy"}
                )
            ...
            self._update_task_progress(task, db_session, "file_analysis", 25.0, "分析文件结构中")
            ...
            # 记录HDFS扫描操作
            table_location = f"/user/hive/warehouse/{task.database_name}.db/{task.table_name}"
            merge_logger.log_hdfs_operation(
                "扫描表目录", table_location, MergePhase.FILE_ANALYSIS,
                stats={
            ...
            self._update_task_progress(
                task, db_session, "file_analysis", 35.0, 
                f"发现{files_before}个文件待合并", 
                total_files_count=files_before
            )
            merge_logger.end_phase(MergePhase.FILE_ANALYSIS, f"分析完成，发现{files_before}个文件")
            
            # 第四阶段：创建临时表（模拟）
            merge_logger.start_phase(MergePhase.TEMP_TABLE_CREATION, "创建临时表")
            self._update_task_progress(task, db_session, "temp_table_creation", 50.0, "创建临时表中")
            ...
            merge_logger.log_sql_execution(create_sql, MergePhase.TEMP_TABLE_CREATION, 
                                         success=True, affected_rows=result['files_before'] * 1000)  # 估算行数
            
            # 记录YARN任务监控（如果有）
            if self.yarn_monitor:
                yarn_app_id = f"application_1735876200000_001{task.id}"
                self._update_task_progress(task, db_session, yarn_application_id=yarn_app_id)
                merge_logger.log_yarn_monitoring(
                    yarn_app_id, MergePhase.TEMP_TABLE_CREATION,
                    progress=45.0, state="RUNNING",
                    details={"job_type": "CREATE_TABLE", "estimated_duration": "2m"}
                )
            merge_logger.end_phase(MergePhase.TEMP_TABLE_CREATION, f"临时表{temp_table_name}创建成功")
            
            # 第五阶段：数据验证
            merge_logger.start_phase(MergePhase.DATA_VALIDATION, "验证数据完整性")
            self._update_task_progress(task, db_session, "data_validation", 65.0, "验证数据完整性")
            ...
            merge_logger.log_data_validation(
                MergePhase.DATA_VALIDATION, "数据完整性检查",
                original_count=files_before, new_count=files_after, success=True
            )
            
            self._update_task_progress(
                task, db_session, "data_validation", 75.0,
            ...
            self._update_task_progress(task, db_session, "atomic_swap", 85.0, "执行原子表切换")
            ...
            for sql in swap_sqls:
                merge_logger.log_sql_execution(sql, MergePhase.ATOMIC_SWAP, success=True)
            ...
            if files_before > files_after:
                result['size_saved'] = (files_before - files_after) * 64 * 1024 * 1024  # 估算
            
            result['success'] = True
            result['duration'] = time.time() - start_time
            result['message'] = f'演示合并成功完成。文件数从 {files_before} 减少到 {files_after}'
            ...
            self._update_task_progress(
                task, db_session, "completion", 100.0,
            ...
            merge_logger.log_task_completion(False, int((time.time() - start_time) * 1000))
            db_session.commit()
    ...
    def _get_real_file_count(self, task: MergeTask) -> int:
        ...
        try:
            ...
            # 查询表文件统计
            metric = Session.object_session(task).query(TableMetric).filter_by(
            ...
            if metric and metric.total_files:
                ...
                with self.metastore_connector as conn:
                    table_info = conn.get_table_info(task.database_name, task.table_name)
                    if table_info and 'location' in table_info:
                        hdfs_path = table_info['location'].replace('hdfs://nameservice1', '')
                        stats = self.webhdfs_client.scan_directory_stats(hdfs_path, self.cluster.small_file_threshold)
    ...
    def _update_task_progress(self, task: MergeTask, db_session: Session, 
        ...
        try:
            ...
            db_session.commit()
            logger.info(f"Updated task {task.id} progress: {execution_phase} - {progress_percentage}%")
        except Exception as e:
