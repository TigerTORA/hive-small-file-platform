...
class MergeEngineFactory:
    ...
    @classmethod
    def get_engine(
        cls, cluster: Cluster, engine_type: Optional[str] = None
    ) -> BaseMergeEngine:
        """
        获取合并引擎实例
        统一使用SafeHiveMergeEngine，支持智能策略选择
    ...
    def validate_strategy_compatibility(
        cls, merge_strategy: str, table_format: str = None
    ) -> Dict[str, any]:
        """
        验证策略兼容性
        ...
        if merge_strategy not in capabilities.get("supported_strategies", []):
            ...
            result["warnings"].append(
                f"SafeHiveMergeEngine does not support strategy {merge_strategy}"
            )
            return result
        ...
        if table_format:
            ...
            if merge_strategy == "concatenate" and format_upper not in [
                "TEXTFILE",
                "SEQUENCEFILE",
                "RCFILE",
            ]:
                result["warnings"].append(
                    f"CONCATENATE strategy may not work optimally with {table_format} format"
                )
                result["recommendations"].append(
                    "Consider using INSERT OVERWRITE or SAFE_MERGE strategy instead"
                )

            if merge_strategy == "insert_overwrite" and format_upper in [
                "PARQUET",
                "ORC",
            ]:
                result["recommendations"].append(
                    "For large tables, consider using SAFE_MERGE for zero-downtime operation"
                )

        return result

    @classmethod
    def create_smart_merge_task(
        cls,
        cluster: Cluster,
        database_name: str,
        table_name: str,
        ...
        # 验证策略兼容性
        validation = cls.validate_strategy_compatibility(
            recommended_strategy, table_format
        )
        ...
        return {
            ...
            "strategy_reason": cls._get_strategy_reason(
                recommended_strategy, file_count, table_format, table_size
            ),
        }

    @classmethod
    def _get_strategy_reason(
        cls, strategy: str, file_count: int, table_format: str, table_size: int
    ) -> str:
        """
        获取策略选择原因
        """
        ...
        if strategy == "insert_overwrite":
            ...
            if (table_size and table_size < 256 * 1024 * 1024) or (
                file_count and file_count < 100
            ):
