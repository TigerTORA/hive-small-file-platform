...
def scan_all_clusters(self):
    ...
    try:
        ...
        clusters = (
            db.query(Cluster)
            .filter(Cluster.scan_enabled == True, Cluster.status == "active")
            .all()
        )
        ...
        for cluster in clusters:
            try:
                ...
                logger.info(
                    f"Started scan task for cluster {cluster.name}: {result.id}"
                )
...
def scan_single_cluster(self, cluster_id: int, database_filter: Optional[str] = None):
    ...
    try:
        ...
        # 更新任务状态
        current_task.update_state(
            state="PROGRESS", meta={"status": "initializing", "cluster": cluster.name}
        )
        ...
        for db_name in databases:
            try:
                db_res = scanner.scan_database_tables(
                    db, db_name, table_filter=None, max_tables=None, strict_real=True
                )
                ...
                per_db.append({"database": db_name, "error": str(e)})
...
def scan_single_table(self, cluster_id: int, database_name: str, table_name: str):
    ...
    try:
        ...
        logger.info(
            f"Starting scan for table {database_name}.{table_name} in cluster {cluster.name}"
        )
