...
def cleanup_old_logs(days_to_keep: int = 30):
    ...
    try:
        ...
        # 删除过期的任务日志
        deleted_logs = (
            db.query(TaskLog).filter(TaskLog.timestamp < cutoff_date).delete()
        )

        db.commit()

        logger.info(
            f"Cleaned up {deleted_logs} old task logs (older than {days_to_keep} days)"
        )
...
def cleanup_old_metrics(days_to_keep: int = 90):
    ...
    try:
        ...
        # 删除过期的分区指标
        deleted_partition_metrics = (
            db.query(PartitionMetric)
            .join(TableMetric)
            .filter(TableMetric.scan_time < cutoff_date)
            .delete()
        )

        # 删除过期的表指标
        deleted_table_metrics = (
            db.query(TableMetric).filter(TableMetric.scan_time < cutoff_date).delete()
        )

        db.commit()

        logger.info(
            f"Cleaned up {deleted_table_metrics} table metrics and "
...
def cleanup_completed_tasks(days_to_keep: int = 60):
    ...
    try:
        ...
        if task_ids:
            ...
            deleted_tasks = (
                db.query(MergeTask)
                .filter(MergeTask.id.in_(task_ids))
                .delete(synchronize_session=False)
            )
            ...
            logger.info(
                f"Cleaned up {deleted_tasks} completed tasks and "
...
def system_health_check():
    ...
    health_status = {
        ...
        "redis": "unknown",
        "clusters": [],
        "tasks": {"pending": 0, "running": 0, "failed_recent": 0},
    }
    ...
    try:
        ...
        for cluster in clusters:
            cluster_status = {
                ...
                "last_update": (
                    cluster.updated_time.isoformat() if cluster.updated_time else None
                ),
            }
            health_status["clusters"].append(cluster_status)

        # 检查任务状态
        health_status["tasks"]["pending"] = (
            db.query(MergeTask).filter(MergeTask.status == "pending").count()
        )

        health_status["tasks"]["running"] = (
            db.query(MergeTask).filter(MergeTask.status == "running").count()
        )

        # 最近24小时失败的任务数
        recent_cutoff = datetime.utcnow() - timedelta(hours=24)
        health_status["tasks"]["failed_recent"] = (
            db.query(MergeTask)
            .filter(
                MergeTask.status == "failed", MergeTask.completed_time >= recent_cutoff
            )
            .count()
        )
...
def generate_daily_report():
    ...
    try:
        ...
        scanned_tables = (
            ...
            .filter(
                TableMetric.scan_time >= start_time, TableMetric.scan_time <= end_time
            )
            .count()
        )
        ...
        report = {
            ...
            "scanning": {"tables_scanned": scanned_tables},
            "merge_tasks": {
                "total_completed": len(tasks_completed),
                "successful": len(successful_tasks),
                "failed": len(failed_tasks),
                "success_rate": (
                    len(successful_tasks) / len(tasks_completed) * 100
                    if tasks_completed
                    else 0
                ),
            },
            "merge_effectiveness": {
                ...
                "reduction_rate": (
                    (total_files_before - total_files_after) / total_files_before * 100
                    if total_files_before
                    else 0
                ),
            },
        }
        ...
        logger.info(
            f"Daily report generated for {yesterday}: "
        ...
        return {"date": yesterday.isoformat(), "error": str(e)}
