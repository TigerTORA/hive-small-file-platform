...
def test_webhdfs_connection():
    ...
    for config in test_configs:
        ...
        try:
            scanner = WebHDFSScanner(config['url'], user="hdfs", webhdfs_port=config['port'])
                    ...
                    for test_path in test_paths:
                        ...
                        try:
                            ...
                            if not stats.get('error'):
                                ...
                                logger.info(f"  总大小: {stats['total_size'] / (1024**3):.2f} GB")
                                logger.info(f"  平均文件大小: {stats['avg_file_size'] / (1024**2):.2f} MB")
                                logger.info(f"  扫描时间: {stats['scan_duration']:.2f} 秒")
                            else:
