...
def test_real_scanning():
    ...
    try:
        ...
        if hdfs.get('status') == 'success':
            ...
            if mode == 'mock':
                logger.info(f"  🔄 注意: 使用Mock模式，真实HDFS连接失败: {hdfs.get('real_hdfs_error')}")
            else:
        ...
        # 查询并展示扫描结果
        logger.info("\n📋 扫描详细结果:")
        table_metrics = db_session.query(TableMetric).filter(
        ...
        for metric in table_metrics:
            ...
            logger.info(f"    小文件比例: {(metric.small_files/metric.total_files*100):.1f}%" if metric.total_files > 0 else "N/A")
            ...
            if metric.is_partitioned:
                ...
                # 显示分区详情
                partitions = db_session.query(PartitionMetric).filter(
                    PartitionMetric.table_metric_id == metric.id
                ).all()
                ...
                for partition in partitions[:3]:  # 只显示前3个分区
                    ...
                    logger.info(f"        大小: {partition.total_size / (1024**2):.2f} MB")
